...
TEST_SUITE("Tau API - witness stability (#89)") {
	...
	TEST_CASE("free-region witness does not depend on prior activity") {
		...
		// The canonical free-region choice moves whenever node hashing
		// changes (a parser regen renumbering nonterminals, or the node
		// hash itself), since clause order follows it. Re-pinned after
		// 2f7c3ce9 (nonterminals hashed by name): a fresh process gives
		// 7, 0, 0 for spec7 and 50, 50, 50 for spec50 on their own
		// (verified by driving each alone through the spec-file runner),
		// and so must these post-activity runs. Re-pin whenever the
		// hashing changes; the property under test is fresh ==
