...
TEST_SUITE("Alignments y3") {
	...
	TEST_CASE("simple_conditional_case") {
		tref spec = create_spec("(always i1[t]:bv =_ { 1 } ? o1[t]:bv =_ { 0 } : o1[t]:bv =_ { 1 }).");
		CHECK(transform_to_execution<node_t>(spec) != tau::_F());
	}

	TEST_CASE("simple_andreis_test_case") {
		tref spec = create_spec("always (((i1[t] + i2[t]) <=_ ((i1[t] + i2[t]) >> { 3 }:bv)) ? o1[t] =_ (i1[t] + i2[t]) : o1[t] =_ ~(i1[t] + i2[t])).");
		CHECK(transform_to_execution<node_t>(spec) != tau::_F());
	}
}
