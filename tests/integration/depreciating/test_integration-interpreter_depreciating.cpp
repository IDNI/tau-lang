...
struct output_console {
	...
	std::optional<std::string> type_of(const tau_<BAs...>&) const {
		return { _type };
	}

	void add_output (const tau_<BAs...>& v, const std::string&, const std::string&) {
...
struct input_vector {
	...
	void add_input (const tau_<BAs...>& v, const std::string&, const std::string&) {
		if (!streams.contains(v)) streams.emplace(v,v);
	}
...
TEST_SUITE("Execution") {
	TEST_CASE("o1[t] = i1[t]") {
		...
		std::vector<std::map<tau_<base_bas>, tau_<base_bas>>> assgn_in;
		std::map<tau_<base_bas>, tau_<base_bas>> assgn_out;
...
TEST_SUITE("with inputs and outputs") {
	...
	input_vector<tau_ba<sbf_ba>, sbf_ba> build_i1_inputs(
			std::vector<tau_<tau_ba<sbf_ba>, sbf_ba>> values) {
...
TEST_SUITE("test inputs") {
	...
	TEST_CASE("reading from file with sbf inputs") {
		std::map<tau_<tau_ba<sbf_ba>, sbf_ba>, std::pair<std::string, std::string>> input_map;
...
TEST_SUITE("test outputs") {
	...
	TEST_CASE("writing to file") {
		std::map<tau_<tau_ba<sbf_ba>, sbf_ba>, std::pair<std::string, std::string>> output_map;
