...
TEST_SUITE("adt registry") {
	...
	TEST_CASE("tuple flat order with inheritance") {
		auto r = reg("type Tagged = {tag: bv[8]}. type Point = {a: bool, b: bool}."
	...
	TEST_CASE("alias of tuple is a tuple") {
		auto r = reg("type Point = {a: bool, b: bool}. type P2 = Point. x = 0.");
		REQUIRE(r); CHECK(r->is_tuple(sid("P2")));
	}
	TEST_CASE("duplicate member via inheritance fails") {
		CHECK_FALSE(reg("type A = {m: bool}. type B of (A) is {m: bool}. x = 0."));
	}
	TEST_CASE("cycle fails")             { CHECK_FALSE(reg("type A = {m: B}. type B = {m: A}. x = 0.")); }
	TEST_CASE("self-cycle fails")        { CHECK_FALSE(reg("type A = {m: A}. x = 0.")); }
	TEST_CASE("self alias cycle fails")  { CHECK_FALSE(reg("type X = X. x = 0.")); }
	TEST_CASE("redeclared type resolves to the later definition") {
		auto r = reg("type A = {m: bool}. type A = {n: bool}. x = 0.");
		REQUIRE(r);
		const auto& ms = r->members(sid("A"));
		REQUIRE(ms.size() == 1);
		CHECK(ms[0].path == std::vector<size_t>{sid("n")});
	}
	TEST_CASE("inherit from alias-of-base fails") { CHECK_FALSE(reg("type byte = bv[8]. type A of (byte) is {m: bool}. x = 0.")); }
	...
	TEST_CASE("inherit from an undeclared type fails") {
		CHECK_FALSE(reg("type A of (Nope) is {m: bool}. x = 0."));
	}
	TEST_CASE("two parents: members in declaration order") {
		auto r = reg("type P1 = {a: bool}. type P2 = {b: bool}. "
			"type C of (P1, P2) is {c: bool}. x = 0.");
		REQUIRE(r);
	...
	TEST_CASE("duplicate member across two parents fails") {
		CHECK_FALSE(reg("type P1 = {m: bool}. type P2 = {m: bool}. "
			"type C of (P1, P2) is {c: bool}. x = 0."));
	}
	...
	TEST_CASE("parent cycle fails") {
		CHECK_FALSE(reg("type A of (B) is {a: bool}. type B of (A) is {b: bool}. x = 0."));
	}
	TEST_CASE("duplicate member declared directly fails") {
		CHECK_FALSE(reg("type A = {m: bool, m: bool}. x = 0."));
	}
