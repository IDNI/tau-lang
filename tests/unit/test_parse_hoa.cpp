...
// the ABA oracle skipped. It is now refused via an error on the returned
...
TEST_SUITE("parse_hoa") {
	...
	TEST_CASE("[HOA-03] garbage state count is refused") {
		auto r = parse_hoa("HOA: v1\nStates: garbage\n--BODY--\n--END--\n");
		CHECK(r.has_error());
		CHECK(!r.has_value());
	}

	TEST_CASE("[HOA-04] negative and zero state counts are refused") {
		auto r1 = parse_hoa("HOA: v1\nStates: -2\n--BODY--\n--END--\n");
		CHECK(r1.has_error());
		CHECK(!r1.has_value());
		auto r2 = parse_hoa("HOA: v1\nStates: 0\n--BODY--\n--END--\n");
		CHECK(r2.has_error());
		CHECK(!r2.has_value());
	}
	...
	TEST_CASE("[HOA-06] no States: header is refused") {
		...
		CHECK(r2.has_error());
		CHECK(!r2.has_value());
		auto r3 = parse_hoa("this is not a verdict\n");
		CHECK(r3.has_error());
		CHECK(!r3.has_value());
	}
