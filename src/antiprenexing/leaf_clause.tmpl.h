	...
	const typename term_handle<node>::order& order)
{
	...
	// qlt_dlo_qe's AN-1 rules are sound for: a determined non-empty
	...
	if (is_omcat_type_family<node>(clause_type)) {
		...
		if (has_ordering) {
			...
			while (!vars.empty()) {
				...
				if (auto interval = qlt_dlo_qe<node>(v,
					scoped_v); interval) {
					if (interval.value().is_empty())
						return _F<node>();
