...
tref anti_prenex_block(tref formula, const trefs& block,
	...
	return resolve_quantifiers2<node>(
		eliminate_block_over_clause<node>(formula, block, elim, order),
		order, el);
}
			...
			if (is_bv_type_family<node>(tau::get(var).get_ba_type())) {
				...
				if (bv_blasting
					...
					if (auto blasted = bv_predicate_blasting<node>(n);
						...
						return blasted;
				excluded.insert(n);
			} else if (is_omcat_type_family<node>(
					tau::get(var).get_ba_type())) {
				// qlt (dense linear order over the rationals) is decided
				// by DLO quantifier elimination, not by the atomless-BA
				// path, which cannot reason about ordering atoms. Only a
				// closed scope can be settled to T/F here.
				if (const trefs& free_vars = get_free_vars<node>(n);
					free_vars.empty()) {
					tref body = tau::get(n)[0].second();
					if (auto interval = qlt_dlo_qe<node>(var, body);
						interval) {
						if (tau::get(n).child_is(tau::wff_ex))
							// ex var. body: T iff the satisfying
							// interval is non-empty
							return interval.value().is_empty()
								? tau::_F() : tau::_T();
						else
							// all var. body: T iff the satisfying
							// interval covers the full domain
							return interval.value().is_full()
								? tau::_T() : tau::_F();
					}
