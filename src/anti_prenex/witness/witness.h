 ...
 * `find_pin_for(atom, x)` (simplify.h): the equation that solves a variable,
 * asked about the variable being eliminated. The COF mode asks `cof`
 * (shared/cofactors.h) for the same test on the cofactors the term's BDD
 * hands over (§6). Neither reads an equation's spelling: `x = t`, `t = x`
 * and `x + t = 0` are one term `TERM_OF(c)` to them. An equation under `¬`
 * pins nothing in the ∃ sense, and is exactly what pins in the ∀ sense,
 * where the deep pass works under `¬∃x¬`.
 *
 * NO BDD IN THE SPELLED MODE (§3). The entries WITHOUT a `ctx` — phase 2's —
 * Debug-assert that their formula carries no `BDD_ID` anywhere; a backed
 * term reaching one is a caller bug. The entries WITH a `ctx` work on
 * conjuncts whose terms are BDD-backed under `ctx.order`, and the BDD
 * primitives assert the order themselves. Nothing here spells a term out.
 *
 * WHAT MAY TOUCH A UNIT (§4) — a surviving binder, opaque to every step: the
 * substitution of `x` DESCENDS into its body, because the pinning conjunct
 * that licensed the substitution is consumed and an untouched inner
 * occurrence would desynchronise from the outer rewrite. Phase 0's binder
 * ids make that descent capture-safe, and no bound variable is ever renamed.
 * What is opaque is the MATCH: a unit is no conjunction and no disjunction,
 * so it is never a pin and never a case-pin branch.
 *
 * SIMPLIFICATION: `TRY_WITNESS` simplifies its result (invariant 6); the deep
 * pass does NOT, phase 2's global `SIMPLIFY` follows it (§3).
 */
 ...
 * `nullopt`.
 *
 * The top-level conjuncts that are POSITIVE equations are scanned — an
 * equation under `¬` pins nothing in the ∃ sense and an order atom never
 * pins — and on each the term `TERM_OF(c)` is cofactored by `x` through
 * `cof`: `x` is pinned iff the record is usable and `f₀ ∪ f₁ = 1`, the
 * witness is `f₁′`, and the pin is STRICT when `p` folds to `0`. A conjunct
 * whose term does not mention `x` is skipped; one that hides `x` inside a
 * leaf is `usable = false` (§1, the LEAF HAZARD) and pins nothing either.
 * What follows the match is the spelled mode's: a STRICT pin first, else the
 * smallest `‖f₁′‖`; the witness substituted for `x` in EVERY conjunct, THE
 * PINNING ONE INCLUDED; the result `SIMPLIFY`d (invariant 6).
 */
template <NodeType node>
 ...
 *
 * `dᵢ` is the branch AS IT STANDS, its pinning conjunct included: the pin
 * becomes the branch's residual under `[x ← tᵢ]` (§3 `TRY_WITNESS`). `rest`
 * is the ∧-join of `ψ`'s other conjuncts. No copy of the rewrite is built
 * here; the caller emits it branch by branch, so a deciding branch is the
 * only copy paid for.
 */
template <NodeType node>
struct case_witness {
 ...
 * qualifies.
 *
 * The ∃ form; `try_witness_deep` matches the same shape dualised.
 */
template <NodeType node>
