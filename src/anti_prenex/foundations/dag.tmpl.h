...
//   (binary), build_wff_neg; ctx.h's memoised for size_memo, neg_memo,
//   negative_tree_memo (lazy fill; the accessor is the only writer — on an
//   unconditional table `memoised` computes once and returns a reference
//   into the table; `find`/`store` remain available for a split fill like
//   neg_of / set_neg).
...
//   reached through ctx.h's memoised/find/store — never a private
//   `#ifdef TAU_CACHE` table here (fwd.h, cache gating). `members` is NOT
//   cached: a linear walk of the chain, returned by value (ruling Sep 7).
