...
/**
 * @file prims.h
 * @brief Anti-prenexing foundations: the formula-level primitives of §3 —
 ...
 * `tree<node>::substitute`). Ids are not unique — siblings share one — and
 * need not be; strict decrease along a path is the whole property. Numbering
 * starts at 1 and ignores free variables that are themselves numerically
 * named: such names arise only from this pass, the module never introduces a
 * binder that was not already there, and strict decrease keeps a fragment's
 * ids below every enclosing one.
 *
 ...
 * derived from its position, so that alpha-variants become one node. This is
 ...
 * order, with `X[0]` outermost.
 *
 * NO narrowing happens here: `X` is attached as given. Bound variables keep
 * their ids (`calculate_quant_id = false`). Nothing is published: `|·|` and
 * `FV` are computed lazily on the first query. `X` must be duplicate-free,
 * since a repeated variable would build a vacuous inner binder and a
 * non-canonical memo key. An empty `X` returns `φ`.
 */
template <NodeType node>
 ...
 * binder of `φ`, in one memoised pass.
 */
template <NodeType node>
