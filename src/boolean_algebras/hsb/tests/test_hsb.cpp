...
TEST_SUITE("hsb — dispatcher") {
	...
	TEST_CASE("hsb type is recognized") {
		...
		CHECK(ba_descriptor<hsb, node_t>::matches_type(tt) == true);
	}

	TEST_CASE("hsb type is not qint type") {
		gc_fixture gc;
		auto tt = hsb_type<node_t>();
		CHECK(ba_descriptor<qint, node_t>::matches_type(tt) == false);
	}
...
TEST_SUITE("hsb — LTL integration") {
	...
	TEST_CASE("G(o1:hsb = {bot}:hsb) & F(o1:hsb != {bot}:hsb) is UNREALIZABLE") {
		...
		REQUIRE(fm != nullptr);
		{
			// Full-LTL (has F) and unrealizable: sat is realizability
			// (README "Satisfiability"), so it decides false.
			auto sat = is_tau_formula_sat<node_t>(fm);
			REQUIRE(sat.has_value());
