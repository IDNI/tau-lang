 ...
 *
 * `needed_width` computes, bottom-up over a single `bf` (bitvector term)
 * subtree, the width wide enough to evaluate it without silently
 * overflowing; `widen_bv_arithmetic` elaborates a whole formula with it.
 *
...
#define __IDNI__TAU__BOOLEAN_ALGEBRAS__BV__BV_WIDENING_H__
 ...
 * `bv_widening` runtime flag (declared above) is `false`, or
 ...
 * // only when it is set (see boolean_algebras/bv/tests/test_bv_widening.cpp, "mul
...
#endif // __IDNI__TAU__BOOLEAN_ALGEBRAS__BV__BV_WIDENING_H__
