...
// lookup requires a declaration visible at this point.
int compare_bv_consts(const bv& c1, const bv& c2);
...
tref term_add(tref symbol) {
	...
	auto add_consts = [&](const bv& c1, const bv& c2, size_t type_id) -> tref {
		...
		if (bv_widening && compare_bv_consts(res, c1) < 0) return symbol;
...
tref term_sub(tref symbol) {
	...
	auto sub_consts = [&](const bv& c1, const bv& c2, size_t type_id) -> tref {
		...
		// of wrapping now.
		if (bv_widening && compare_bv_consts(c1, c2) < 0) return symbol;
