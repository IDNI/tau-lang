...
std::optional<bv> bv_eval_node(cvc5::Solver& solver, const typename tree<node>::traverser& form, subtree_map<node, bv> vars,
	...
	switch (nt) {
		...
		case node::type::wff_sometimes: {
			return bv_eval_node<node>(solver, form | tt::first, vars, free_vars);
		}
		...
		case node::type::wff_all: {
			...
			auto f = bv_eval_node<node>(solver, form | tt::second, vars, free_vars);
...
bool is_bv_formula_sat(tref form) {
	...
	auto expr = bv_eval_node<node>(solver, tt(form), vars, free_vars);
...
std::optional<solution<node>> solve_bv(const tref form) {
	...
	if (result.isSat()) {
		...
		for (const auto& [tau_var, bv_var] : free_vars) {
			...
			s.emplace(tau::get(tau::bf, tau_var), tau::get(tau::bf, tau::get_ba_constant(cte, "bv")));
		}
		return s;
	} else
