	...
	switch (nt) {
		...
		case node::type::wff_all: {
			...
			for (const auto& v: (form | tt::first | tt::Tree).select_top(is<node, tau::variable>)) {
				...
				auto x = cvc5_solver.mkVar(cvc5_solver.mkBitVectorSort(bv_size), vn.c_str());
		...
		case node::type::variable: {
			...
			auto x = cvc5_solver.mkConst(cvc5_solver.mkBitVectorSort(bv_size), vn.c_str());
			//BOOST_LOG_TRIVIAL(trace) << "cvc5.tmpl.h:" << __LINE__ << " bv_eval_node/x: " << x;
			//vars.emplace(form, x);
		...
		case node::type::bitvector: {
			...
			return cvc5_solver.mkConst(cvc5_solver.mkBitVectorSort(2));
			// by replacing with
