...
static constexpr const auto& get_children =
		traverser<sbf_sym, sbf_parser>::get_children_extractor();
static constexpr const auto& get_only_child =
		traverser<sbf_sym, sbf_parser>::get_only_child_extractor();
static constexpr const auto& get_terminals =
		traverser<sbf_sym, sbf_parser>::get_terminal_extractor();
static constexpr const auto& get_nonterminal =
		traverser<sbf_sym, sbf_parser>::get_nonterminal_extractor();

// evaluates a parsed bdd terminal node recursively
sbf_ba eval_node(const traverser<sbf_sym, sbf_parser>& t) {
	...
	const std::string& src)
{
	...
	auto t = traverser<sbf_sym, sbf_parser>(root) | sbf_parser::sbf;
