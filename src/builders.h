...
builder<BAs...> make_builder(const tau_depreciating<BAs...>& builder) {
...
template <typename... BAs>
tau_depreciating<BAs...> tau_apply_builder(const builder<BAs...>& b,
	std::vector<tau_depreciating<BAs...>>& n)
{
	std::map<tau_depreciating<BAs...>, tau_depreciating<BAs...>> changes;
	std::vector<tau_depreciating<BAs...>> vars = b.first || tau_parser::capture;
...
template <typename... BAs>
static const tau_depreciating<BAs...> _0 = bldr_bf_0<BAs...>.second;

template <typename... BAs>
static const tau_depreciating<BAs...> _0_trimmed = trim(_0<BAs...>);

template <typename... BAs>
static const tau_depreciating<BAs...> _1 = bldr_bf_1<BAs...>.second;

template <typename... BAs>
static const tau_depreciating<BAs...> _1_trimmed = trim(_1<BAs...>);

template <typename... BAs>
static const tau_depreciating<BAs...> _F = bldr_wff_F<BAs...>.second;

template <typename... BAs>
static const tau_depreciating<BAs...> _F_trimmed = trim(_F<BAs...>);

template <typename... BAs>
static const tau_depreciating<BAs...> _T = bldr_wff_T<BAs...>.second;

template <typename... BAs>
static const tau_depreciating<BAs...> _T_trimmed = trim(_T<BAs...>);
...
template<typename... BAs>
tau_depreciating<BAs...> build_int(int_t i) {
	using p = tau_parser;
...
template <typename... BAs>
tau_depreciating<BAs...> build_variable(const std::string& name) {
	return wrap<BAs...>(tau_parser::variable, name);
}

template <typename... BAs>
tau_depreciating<BAs...> build_in_var_name(const size_t& index) {
...
template<typename... BAs>
tau_depreciating<BAs...> build_type(const std::string& type) {
	return wrap<BAs...>(tau_parser::type, type);
}

template<typename... BAs>
tau_depreciating<BAs...> build_bf_t_type(const std::string& type) {
	return wrap(
		tau_parser::bf, wrap(
			tau_parser::bf_t,
				build_type<BAs...>(type)));
...
template<typename... BAs>
tau_depreciating<BAs...> build_bf_t_type(const tau_depreciating<BAs...>& type) {
	return wrap(
		tau_parser::bf, wrap(
			tau_parser::bf_t,
				type));
...
template<typename... BAs>
tau_depreciating<BAs...> build_bf_f_type(const std::string& type) {
	return wrap(
		tau_parser::bf, wrap(
			tau_parser::bf_f,
				build_type<BAs...>(type)));
...
template<typename... BAs>
tau_depreciating<BAs...> build_bf_f_type(const tau_depreciating<BAs...>& type) {
	return wrap(
		tau_parser::bf, wrap(
			tau_parser::bf_f,
				type));
...
template<typename... BAs>
tau_depreciating<BAs...> build_in_variable_at_n(const tau_depreciating<BAs...>& in_var_name, const size_t& num) {
...
template <typename... BAs>
tau_depreciating<BAs...> build_in_variable_at_n(const size_t& index, const size_t& num) {
	return build_in_variable_at_n(build_in_var_name<BAs...>(index), num);
}

template<typename... BAs>
tau_depreciating<BAs...> build_in_variable_at_n(const std::string& name, const int_t pos) {
	using p = tau_parser;
...
template <typename... BAs>
tau_depreciating<BAs...> build_in_variable_at_t(const tau_depreciating<BAs...>& in_var_name) {
	assert(is_non_terminal(tau_parser::in_var_name, in_var_name));
...
template <typename... BAs>
tau_depreciating<BAs...> build_in_variable_at_t(const size_t& index) {
	return build_in_variable_at_t(build_in_var_name<BAs...>(index));
}

template <typename... BAs>
tau_depreciating<BAs...> build_in_variable_at_t_minus (const std::string& name, const int_t shift) {
	using p = tau_parser;
	auto var_name = wrap<BAs...>(p::in_var_name, name);
...
template <typename... BAs>
tau_depreciating<BAs...> build_in_variable_at_t_minus(const tau_depreciating<BAs...>& in_var_name, const size_t& num) {
	assert(is_non_terminal(tau_parser::in_var_name, in_var_name));
	assert(num > 0);
...
template <typename... BAs>
tau_depreciating<BAs...> build_in_variable_at_t_minus(const size_t& index, const size_t& num) {
	return build_in_variable_at_t_minus(build_in_var_name<BAs...>(index), num);
}

template <typename... BAs>
tau_depreciating<BAs...> build_out_var_name(const size_t& index) {
	std::stringstream var_name;
	var_name << "o" << index;
...
template <typename... BAs>
tau_depreciating<BAs...> build_out_var_name(const std::string& name) {
	return wrap<BAs...>(tau_parser::out_var_name, name);
}

template <typename... BAs>
tau_depreciating<BAs...> build_out_variable_at_t(const tau_depreciating<BAs...>& out_var_name) {
...
template <typename... BAs>
tau_depreciating<BAs...> build_out_variable_at_t(const size_t& index) {
	return build_out_variable_at_t(build_out_var_name<BAs...>(index));
}

template <typename... BAs>
tau_depreciating<BAs...> build_out_variable_at_n(const tau_depreciating<BAs...>& out_var_name, const size_t& num) {
	assert(is_non_terminal(tau_parser::out_var_name, out_var_name));
...
template <typename... BAs>
tau_depreciating<BAs...> build_out_variable_at_n(const size_t& index, const size_t& num) {
	return build_out_variable_at_n(build_out_var_name<BAs...>(index), num);
}

template<typename... BAs>
tau_depreciating<BAs...> build_out_variable_at_n(const std::string& name, const int_t pos) {
	using p = tau_parser;
...
template <typename... BAs>
tau_depreciating<BAs...> build_out_variable_at_t_minus (const std::string& name, const int_t shift) {
	using p = tau_parser;
	auto var_name = wrap<BAs...>(p::out_var_name, name);
	auto shift_node = wrap<BAs...>(p::shift, {wrap<BAs...>(p::variable, "t"), build_num<BAs...>(shift)});
	auto offset = wrap<BAs...>(p::offset, shift_node);
...
template <typename... BAs>
tau_depreciating<BAs...> build_out_variable_at_t_minus(const tau_depreciating<BAs...>& out_var_name, const size_t& num) {
	assert(is_non_terminal(tau_parser::out_var_name, out_var_name));
	assert(num > 0);
...
template <typename... BAs>
tau_depreciating<BAs...> build_out_variable_at_t_minus(const size_t& index, const size_t& num) {
	return build_out_variable_at_t_minus(build_out_var_name<BAs...>(index), num);
}

// ------ Helpers for variables having io_var as child ---------------
template <typename... BAs>
auto is_io_initial (const tau_depreciating<BAs...>& io_var) {
	return (trim2(io_var)->child[1] | tau_parser::integer).has_value();
}

template <typename... BAs>
auto is_io_shift (const tau_depreciating<BAs...>& io_var) {
	return (trim2(io_var)->child[1] | tau_parser::shift).has_value();
}

template <typename... BAs>
auto get_io_time_point (const tau_depreciating<BAs...>& io_var) {
	return int_extractor<BAs...>(trim(trim2(io_var)->child[1]));
}

template <typename... BAs>
auto get_io_shift (const tau_depreciating<BAs...>& io_var) {
	return size_t_extractor<BAs...>(trim2(io_var)->child[1]->child[0]->child[1]->child[0]).value();
}

template <typename... BAs>
std::string get_io_name (const tau_depreciating<BAs...>& io_var) {
	std::stringstream ss; ss << trim(trim2(io_var));
	return ss.str();
}

template<typename... BAs>
tau_depreciating<BAs...> get_tau_io_name(const tau_depreciating<BAs...>& io_var) {
	return trim(trim2(io_var));
}

template <typename... BAs>
int_t get_io_var_shift(const tau_depreciating<BAs...>& io_var) {
	// If there is a shift
...
template <typename... BAs>
int_t get_max_initial(const std::vector<tau_depreciating<BAs...>>& io_vars) {
	int_t max_init = -1;
	for (const tau_depreciating<BAs...>& v : io_vars) {
		if (is_io_initial(v)) {
...
template <typename... BAs>
tau_depreciating<BAs...> build_bf_var(const std::string& name) {
	auto var = make_builder<BAs...>("( $X ) =: " + name + ".").second;
	return trim<BAs...>(var);
}

template <typename... BAs>
tau_depreciating<BAs...> build_wff_var(const std::string& name) {
	auto var = make_builder<BAs...>("( $X ) =:: ?" + name + ".").second;
	return trim<BAs...>(var);
}

template <typename... BAs>
tau_depreciating<BAs...> build_bf_constant(const tau_depreciating<BAs...>& cte) {
	return wrap(
		tau_parser::bf, wrap(
...
template<typename... BAs>
tau_depreciating<BAs...> build_bf_constant(const tau_depreciating<BAs...>& cte, const tau_depreciating<BAs...>& type) {
	return wrap(
		tau_parser::bf, wrap(
			tau_parser::bf_constant, wrap(
				tau_parser::constant,
...
template<typename... BAs>
tau_depreciating<BAs...> build_bf_constant(const tau_depreciating<BAs...>& cte, const std::string& type) {
...
template<typename... BAs>
tau_depreciating<BAs...> build_bf_and_constant( const auto& ctes, const tau_depreciating<BAs...>& type)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_bf_or_constant( const auto& ctes, const tau_depreciating<BAs...>& type)
{
...
template <typename... BAs>
std::optional<tau_depreciating<BAs...>> build_bf_constant(
...
template <typename... BAs>
tau_depreciating<BAs...> build_bf_uniter_const(const std::string& n1, const std::string& n2) {
...
template <typename... BAs>
tau_depreciating<BAs...> build_wff_eq(const tau_depreciating<BAs...>& l) {
	return wrap(tau_parser::wff, wrap(tau_parser::bf_eq, l, _0<BAs...>));
}

template <typename... BAs>
tau_depreciating<BAs...> build_wff_neq(const tau_depreciating<BAs...>& l) {
	return wrap(tau_parser::wff, wrap(tau_parser::bf_neq, l, _0<BAs...>));
}

template <typename... BAs>
tau_depreciating<BAs...> build_wff_and(const tau_depreciating<BAs...>& l,
...
template <typename... BAs>
tau_depreciating<BAs...> build_wff_and(const auto& wffs) {
	return std::accumulate(wffs.begin(), wffs.end(), _T<BAs...>,
		[](const auto& l, const auto& r) {return build_wff_and<BAs...>(l, r);});
}

template <typename... BAs>
tau_depreciating<BAs...> build_wff_or(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_wff_or(const auto& wffs) {
	return std::accumulate(wffs.begin(), wffs.end(), _F<BAs...>,
		[](const auto& l, const auto& r) { return build_wff_or<BAs...>(l, r);});
}

template <typename... BAs>
tau_depreciating<BAs...> build_wff_neg(const tau_depreciating<BAs...>& l) {
	return wrap(tau_parser::wff, wrap(tau_parser::wff_neg, l));
}

template <typename... BAs>
tau_depreciating<BAs...> build_wff_xor_from_def(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_wff_xor(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
	return build_wff_or(
...
template <typename... BAs>
tau_depreciating<BAs...> build_wff_imply(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_wff_conditional(const tau_depreciating<BAs...>& x,
	const tau_depreciating<BAs...>& y,
	const tau_depreciating<BAs...>& z)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_wff_equiv(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_wff_all(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_wff_ex(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_wff_sometimes(const tau_depreciating<BAs...>& l) {
	return wrap(tau_parser::wff, wrap(tau_parser::wff_sometimes, l));
}

template <typename... BAs>
tau_depreciating<BAs...> build_wff_always(const tau_depreciating<BAs...>& l) {
	return wrap(tau_parser::wff, wrap(tau_parser::wff_always, l));
}

// bf factory method for building bf formulas
template <typename... BAs>
tau_depreciating<BAs...> build_bf_and(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_bf_and(const auto& bfs) {
	return std::accumulate(bfs.begin(), bfs.end(), _1<BAs...>,
		[](const auto& l, const auto& r) { return build_bf_and<BAs...>(l, r);});
}

template <typename... BAs>
tau_depreciating<BAs...> build_bf_or(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_bf_or(const auto& bfs) {
	return std::accumulate(bfs.begin(), bfs.end(), _0<BAs...>,
		[](const auto& l, const auto& r) { return build_bf_or<BAs...>(l, r); });
}

template <typename... BAs>
tau_depreciating<BAs...> build_bf_neg(const tau_depreciating<BAs...>& l) {
	return wrap(tau_parser::bf, wrap(tau_parser::bf_neg, l));
}

template <typename... BAs>
tau_depreciating<BAs...> build_bf_xor_from_def(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_bf_xor(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
	return build_bf_or<BAs...>(
...
template<typename... BAs>
tau_depreciating<BAs...> build_wff_eq(const tau_depreciating<BAs...>& l, const tau_depreciating<BAs...>& r) {
#ifdef DEBUG
	using p = tau_parser;
...
template <typename... BAs>
tau_depreciating<BAs...> build_bf_less(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_bf_nless(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_bf_less_equal(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_bf_nleq(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_bf_interval(const tau_depreciating<BAs...>& x,
	const tau_depreciating<BAs...>& y, const tau_depreciating<BAs...>& z)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_bf_nleq_lower(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
	std::vector<tau_depreciating<BAs...>> args {trim(l), trim(r)};
	return tau_apply_builder<BAs...>(bldr_bf_nleq_lowwer<BAs...>, args);
}

template <typename... BAs>
tau_depreciating<BAs...> build_bf_nleq_upper(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
	std::vector<tau_depreciating<BAs...>> args {trim(l), trim(r)};
...
template <typename... BAs>
tau_depreciating<BAs...> build_bf_greater(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_bf_ngreater(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_bf_greater_equal(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_bf_ngeq(const tau_depreciating<BAs...>& l,
	const tau_depreciating<BAs...>& r)
{
...
template <typename... BAs>
tau_depreciating<BAs...> build_wff_ctn_greater_equal(const tau_depreciating<BAs...>& ctnvar,
	const tau_depreciating<BAs...>& num) {
...
template <typename... BAs>
tau_depreciating<BAs...> build_wff_ctn_greater(const tau_depreciating<BAs...>& ctnvar,
	const tau_depreciating<BAs...>& num) {
	return wrap(tau_parser::wff,
			wrap(tau_parser::constraint,
...
template <typename... BAs>
tau_depreciating<BAs...> build_wff_ctn_less_equal(const tau_depreciating<BAs...>& ctnvar,
	const tau_depreciating<BAs...>& num) {
	return wrap(tau_parser::wff,
			wrap(tau_parser::constraint,
...
template <typename... BAs>
tau_depreciating<BAs...> build_wff_ctn_less(const tau_depreciating<BAs...>& ctnvar,
	const tau_depreciating<BAs...>& num) {
	return wrap(tau_parser::wff,
			wrap(tau_parser::constraint,
...
template <typename... BAs>
tau_depreciating<BAs...> build_wff_ctn_eq(const tau_depreciating<BAs...>& ctnvar,
	const tau_depreciating<BAs...>& num) {
	return wrap(tau_parser::wff,
			wrap(tau_parser::constraint,
...
template <typename... BAs>
tau_depreciating<BAs...> build_wff_ctn_neq(const tau_depreciating<BAs...>& ctnvar,
	const tau_depreciating<BAs...>& num) {
	return wrap(tau_parser::wff,
			wrap(tau_parser::constraint,
