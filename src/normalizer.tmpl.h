...
tref eliminate_bv_and_quantifiers(tref form) {
	...
	// reference at *conjunct* granularity -- eliminate_block_over_clause
	// reserves unrecognised conjuncts (and their variables' components)
	// per conjunct, independent conjuncts are split out before it applies,
	// and
...
bool is_nso_impl(tref n1, tref n2) {
	...
	auto closed_impl_holds = [](tref f, tref g) {
		...
		if (tau::get(nres).find_top(is_quantifier<node>)) {
			...
			// matches EXISTENTIAL scopes, so a universal closure whose
			// variables carry the `arithmetic` verdict (order atoms of
			// a qlt/qint stream, say) survives normalize_non_temp with
			// its binder in place. Its negation is the ∃ form;
			// eliminate that and negate back. Surfaced by the
			// per-component split above on `G (o1[t]:qlt = i1[t]:qlt
			// && o1[t]:qlt > {1}:qlt)`: the component-unsat closure
			// `all b2, b1 !(b2 = b1 && {1}:qlt < b1)` was reported
			// undecided (a conservative "false"), where the previous
			// whole-formula closure never reached normalization.
			tref nb = eliminate_bv_and_quantifiers<node>(
				tau::build_wff_neg(nres));
			if (nb && !tau::get(nb).find_top(is_quantifier<node>))
				nres = normalize_non_temp<node>(tau::build_wff_neg(nb));
		}
	...
	// Orientation: n1 -> n2 and !n2 -> !n1 are the same question, but the
	// component split below only sees TOP-LEVEL conjuncts, and a negated
	// conjunction is one disjunction -- a single conjunct that connects
	// every variable it mentions. Decide in whichever orientation exposes
	// more conjuncts. The interpreter's tau-constant hooks ask exactly the
	// bad case: two complements `{A}' `and `{A'}'` of the same N-clause
	// constant (A' a clause permutation of A, so the syntactic check above
	// misses it) -- closed as `all V (!A -> !A')` that is one 2N-variable
	// disjunction and a 2^N Boole decomposition (the #82 reproducer never
	// reached step 1 through it); as `all V (A' -> A)` every component's
	// consequent is literally in its antecedent and it decides at once.
	{
		auto conjs = [](tref f) {
			return get_cnf_wff_clauses<node>(f).size();
		};
		const tref c1 = to_nnf<node>(tau::build_wff_neg(n2));
		const tref c2 = to_nnf<node>(tau::build_wff_neg(n1));
		if (conjs(c1) + conjs(c2) > conjs(n1) + conjs(n2)) {
			LOG_DEBUG << "is_nso_impl: deciding the contrapositive";
			n1 = c1, n2 = c2;
		}
	}
