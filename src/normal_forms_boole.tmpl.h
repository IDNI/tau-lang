...
tref treat_ex_quantified_clause(tref ex_clause, bool& quant_eliminated) {
	...
	// Theory-specific quantifier elimination, before the atomless-BA path:
	// atomless-BA QE cannot reason about ordering and would answer a pure
	// ordering atom with T. An owner that cannot resolve the body declines,
	// and the atomless-BA path below handles it.
	if (auto sat = pack_omcat_qe<node>(tau::get(var).get_ba_type(),
						var, scoped_fm); sat)
		return *sat ? new_fm : tau::_F();
		...
		if (is_child_quantifier<node>(n)) {
			...
			if (pack_type_has_arith_ops<node>(
				...
				if (const trefs& free_vars = get_free_vars<node>(n);
					free_vars.empty()) {
					// The quantifier node itself is passed: which
					// end of the theory's answer is asked about is
					// the owner's reading of wff_ex / wff_all.
