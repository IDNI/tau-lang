 ...
 * CHECK( tau::get(normalize<node_t>(fm1).value_or(nullptr)).equals_T() );
 *
 ...
 * tref res2 = normalize<node_t>(fm2).value_or(nullptr);
 ...
 * CHECK( has_no_boolean_combs_of_models<node_t>(fm1).value_or(false) );
 *
 * tref fm2 = get_nso_rr(
 *     "(always x = 0) && (always y = 0).").value().main->get();
 * CHECK( !has_no_boolean_combs_of_models<node_t>(fm2).value_or(false) );
 * @endcode
 */
template <NodeType node>
 ...
 * CHECK( is_non_temp_nso_satisfiable<node_t>(fm1).value_or(false) );
 *
 * tref fm2 = get_nso_rr("x = 0 && x != 0.").value().main->get();
 * CHECK( !is_non_temp_nso_satisfiable<node_t>(fm2).value_or(false) );
 * @endcode
 */
template <NodeType node>
...
/// @brief Check whether a non-temporal formula is unsatisfiable.
template <NodeType node>
 ...
 * CHECK( !is_nso_impl<node_t>(n2, n1).value_or(false) );
 * @endcode
 */
template <NodeType node>
 ...
 * CHECK( tau::get(normalize_with_temp_simp<node_t>(fm).value_or(nullptr)).equals_T() );
 * @endcode
 */
template <NodeType node>
 ...
 * tref res = normalizer<node_t>(nso_rr).value_or(nullptr);
 ...
 * @code{.cpp}
 * tref fm = get_nso_rr("x = 0 || x != 0.").value().main->get();
 * CHECK( tau::get(normalizer<node_t>(fm).value_or(nullptr)).equals_T() );
 * @endcode
 */
template <NodeType node>
