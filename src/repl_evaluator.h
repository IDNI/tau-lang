...
enum repl_option { none_opt, invalid_opt, severity_opt, status_opt,
	colors_opt, charvar_opt, blasting_opt, case_split_opt, defelim_opt, stepprop_opt, factoring_opt,
...
struct repl_evaluator {
	...
	struct options {
		...
		bool stepprop            = step_definitional_propagation; ///< Definitional propagation per step; follows the library default.
...
private:
	...
	/// @brief Update the step-propagation option to @p value and return the old value.
	bool update_stepprop(bool value);
