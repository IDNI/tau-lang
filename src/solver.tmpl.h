	...
	const inequality_system<node>& sys, const solver_options& options)
{
	if constexpr (ba_variant_includes_v<qlt, typename tree<node>::constant>) {
		...
		// SO-14: a constants-only ordering system is not automatically
		// SAT -- ground comparisons fold to T/F at construction, so
		// anything not reducing away must actually hold.
		if (all_vars.empty()) {
			for (tref atom : sys)
