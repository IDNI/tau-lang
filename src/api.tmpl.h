        ...
        interpreter<node>& i, std::map<stream_at, std::string> inputs)
{
        ...
        for (auto& [in, value] : inputs) {
                ...
                auto it = std::find_if(ctx.inputs.begin(), ctx.inputs.end(), has_var_name_sid);
                if (it == ctx.inputs.end()) {
                        TAU_LOG_ERROR << "Input stream " << in.first << " not found in context";
                        return {};
                }
        ...
        for (tref step_input : step_inputs) {
                ...
                if (has_open_tau_fm_in_constant<node>(c)) {
                        TAU_LOG_ERROR << "Constant contains an open tau formula: " << input_value;
                        return {};
                }
                values[step_input] = c;
                TAU_LOG_TRACE << "Parsed input `" << input_value << "` : " << TAU_LOG_BA_TYPE(type_id);
                TAU_LOG_TRACE << "Value: " << TAU_LOG_FM_DUMP(c);
        }

        // Step the interpreter
        auto [output, auto_continue] = i.step(values);
        if (!output.has_value()) {
                TAU_LOG_ERROR << "Failed to step interpreter at time point " << i.time_point;
                return {};
        }
        // Write output values
        ...
        for (const auto& [out, val] : output.value()) {
                TAU_LOG_TRACE << "Output " << get_var_name<node>(out) << "[" << i.time_point << "] = `" << tau::get(val).to_str() << "`";
                TAU_LOG_TRACE << TAU_LOG_FM_DUMP(out);
                TAU_LOG_TRACE << TAU_LOG_FM_DUMP(val);
