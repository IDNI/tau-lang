...
namespace idni::tau_lang {
	...
	std::optional<tau<z3_ba>> nso_factory<z3_ba>::parse(const std::string& src,
		const std::string&)
	{
		return bvf.parse(src);
	}

	tau<z3_ba> nso_factory<z3_ba>::binding(const tau<z3_ba>& n,
		const std::string&)
	{
		return bvf.binding(n);
	}

	std::vector<std::string> nso_factory<z3_ba>::types() const {
		return { "sbf" };
	}

	tau<z3_ba> nso_factory<z3_ba>::splitter_one() const {
		throw std::logic_error("Unsupported");
	}

	std::string nso_factory<z3_ba>::default_type() const {
		return "sbf";
	}

	std::string nso_factory<z3_ba>::one(const std::string) const {
		return "1";
	}

	std::string nso_factory<z3_ba>::zero(const std::string) const {
		return "0";
	}

	std::optional<tau<z3_ba>> nso_factory<z3_ba>::unpack_tau_ba(
		const std::variant<z3_ba>&) const {
		// There is no tau_ba present
		return {};
	}

	nso_factory<z3_ba>& nso_factory<z3_ba>::instance() {
		static nso_factory<z3_ba> factory;
		return factory;
	}

	nso_factory<z3_ba>::nso_factory() {};

	std::optional<tau<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba>> nso_factory<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba>::parse(const std::string& src,
		const std::string type_name)
		{
			return bvf.parse(src);
		}

	tau<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba> nso_factory<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba>::binding(const tau<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba>& n,
		const std::string type_name)
	{
		return bvf.binding(n);
	}

	std::vector<std::string> nso_factory<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba>::types() const {
		return { "sbf" };
	}

	tau<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba> nso_factory<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba>::splitter_one(const std::string& type_name) const {
		throw std::logic_error("Unsupported");
	}

	std::string nso_factory<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba>::default_type() const {
		return "tau";
	}

	std::string nso_factory<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba>::one(const std::string type_name) const {
		return "1";
	}

	std::string nso_factory<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba>::zero(const std::string type_name) const {
		return "0";
	}

	std::optional<tau<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba>> nso_factory<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba>::unpack_tau_ba(
		const std::variant<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba>&) const {
		// There is no tau_ba present
		return {};
	}

	nso_factory<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba>& nso_factory<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba>::instance() {
		static nso_factory<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba> factory;
		return factory;
	}

	nso_factory<tau_ba<z3_ba, sbf_ba>, z3_ba, sbf_ba>::nso_factory() {};
