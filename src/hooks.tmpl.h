	...
	size_t len, tref r)
{
	...
	if (len) {
		...
		for (size_t i = 0; i < len; ++i) {
			ss << (i ? ", " : "") << ch[i]
				<< " ( " << tau::get(ch[i]).value /*dump_to_str(false)*/ << " )";
...
tref get_hook<node>::term_or(const node& v, const tref* ch, size_t len, tref r){
	...
	if (arg1(ch).is(tau::bf_t)) return tau::get(arg1_fm(ch).get(), r);
	//RULE(BF_SIMPLIFY_ONE_1, "$X | 1 := 1.")
	if (arg2(ch).is(tau::bf_t)) return tau::get(arg2_fm(ch).get(), r);
	//RULE(BF_SIMPLIFY_ZERO_2, "0 | $X := $X.")
	if (arg1(ch).is(tau::bf_f)) return tau::get(arg2_fm(ch).get(), r);
	//RULE(BF_SIMPLIFY_ZERO_3, "$X | 0 := $X.")
	if (arg2(ch).is(tau::bf_f)) return tau::get(arg1_fm(ch).get(), r);
	//RULE(BF_SIMPLIFY_SELF_1, "$X | $X := $X.")
	if (arg1_fm(ch) == arg2_fm(ch)) return tau::get(arg1_fm(ch).get(), r);
...
tref get_hook<node>::term_and(const node& v, const tref* ch, size_t len, tref r)
{
	...
	if (arg1(ch).is(tau::bf_t)) return tau::get(arg2_fm(ch).get(), r);
	//RULE(BF_SIMPLIFY_ONE_3, "$X & 1 := $X.")
	if (arg2(ch).is(tau::bf_t)) return tau::get(arg1_fm(ch).get(), r);
	//RULE(BF_SIMPLIFY_ZERO_0, "0 & $X := 0.")
	if (arg1(ch).is(tau::bf_f)) return tau::get(arg1_fm(ch).get(), r);
	//RULE(BF_SIMPLIFY_ZERO_1, "$X & 0 := 0.")
	if (arg2(ch).is(tau::bf_f)) return tau::get(arg2_fm(ch).get(), r);
...
tref get_hook<node>::term_xor(const node& v, const tref* ch, size_t len, tref r)
{
	...
	if (arg1(ch).is(tau::bf_t)) return tau::get(tau::build_bf_neg(arg2_fm(ch).get()), r);
	//RULE(BF_SIMPLIFY_ONE_N, "$X ^ 1 := $X'.")
	if (arg2(ch).is(tau::bf_t)) return tau::get(tau::build_bf_neg(arg1_fm(ch).get()), r);
...
tref get_hook<node>::ctn_neg(tref n) {
	...
	case tau::ctn_neq:  return tau::build_wff_ctn_eq(ctnvar, num);
		//RULE(BF_PUSH_NEGATION_INWARDS_3, "($X = $Y)' := $X != $Y.")
	case tau::ctn_eq:   return tau::build_wff_ctn_neq(ctnvar, num);
		//RULE(BF_PUSH_NEGATION_INWARDS_4, "($X >= $Y)' := $X < $Y.")
	case tau::ctn_gteq: return tau::build_wff_ctn_lt(ctnvar, num);
		//RULE(BF_PUSH_NEGATION_INWARDS_5, "($X > $Y)' := $X <= $Y.")
	case tau::ctn_gt:   return tau::build_wff_ctn_lteq(ctnvar, num);
		//RULE(BF_PUSH_NEGATION_INWARDS_6, "($X <= $Y)' := $X > $Y.")
	case tau::ctn_lteq: return tau::build_wff_ctn_gt(ctnvar, num);
		//RULE(BF_PUSH_NEGATION_INWARDS_7, "($X < $Y)' := $X >= $Y.")
	case tau::ctn_lt:   return tau::build_wff_ctn_gteq(ctnvar, num);
	default: return n;
	}
	return nullptr;
}
...
tref get_hook<node>::wff_and(const node& v, const tref* ch, size_t len, tref r) {
	...
	if (arg1(ch).is(tau::wff_t)) return tau::get(arg2_fm(ch).get(), r);
	//RULE(WFF_SIMPLIFY_ONE_3, "$X && T ::= $X.")
	if (arg2(ch).is(tau::wff_t)) return tau::get(arg1_fm(ch).get(), r);
	//RULE(WFF_SIMPLIFY_ZERO_0, "F && $X ::= F.")
	if (arg1(ch).is(tau::wff_f)) return tau::get(arg1_fm(ch).get(), r);
	//RULE(WFF_SIMPLIFY_ZERO_1, "$X && F ::= F.")
	if (arg2(ch).is(tau::wff_f)) return tau::get(arg2_fm(ch).get(), r);
...
tref get_hook<node>::wff_or(const node& v, const tref* ch, size_t len, tref r) {
	...
	if (arg1(ch).is(tau::wff_t)) return tau::get(arg1_fm(ch).get(), r);
	//RULE(WFF_SIMPLIFY_ONE_1, "$X || T ::= T.")
	if (arg2(ch).is(tau::wff_t)) return tau::get(arg2_fm(ch).get(), r);
	//RULE(WFF_SIMPLIFY_ZERO_2, "F || $X ::= $X.")
	if (arg1(ch).is(tau::wff_f)) return tau::get(arg2_fm(ch).get(), r);
	//RULE(WFF_SIMPLIFY_ZERO_3, "$X || F ::= $X.")
	if (arg2(ch).is(tau::wff_f)) return tau::get(arg1_fm(ch).get(), r);
...
tref get_hook<node>::wff_neg(const node& v, const tref* ch, size_t len, tref r) {
	...
	if (arg1(ch).is(tau::wff_t)) return tau::get(tau::_F(), r);
	//RULE(WFF_SIMPLIFY_ZERO_4, "! F ::= T.")
	if (arg1(ch).is(tau::wff_f)) return tau::get(tau::_T(), r);
	//RULE(WFF_ELIM_DOUBLE_NEGATION_0, "! ! $X ::=  $X.")
	if (auto double_neg = arg1_fm(ch)() | tau::wff_neg | tau::wff;
		double_neg) return double_neg.value();
	if (arg1(ch).is(tau::constraint)) return tau::get(ctn_neg(ch[0]), r);
	return tau::get_raw(v, ch, len, r);
}
	...
	[[maybe_unused]] size_t len, [[maybe_unused]] tref r)
{
	...
	if (arg1_fm(ch) == arg2_fm(ch)) return tau::get(tau::_F(), r);
	...
	if (arg2(ch).is(tau::wff_t)) return tau::get(tau::build_wff_neg(arg1_fm(ch).get()), r);
	//RULE(BF_XOR_SIMPLIFY_6, "T ^ $X ::= ! $X.")
	if (arg1(ch).is(tau::wff_t)) return tau::get(tau::build_wff_neg(arg2_fm(ch).get()), r);
...
tref get_hook<node>::wff_eq(const node& v, const tref* ch, size_t len, tref r) {
	...
	if (arg1(ch).is(tau::bf_t) && arg2(ch).is(tau::bf_f)) return tau::get(tau::_F(), r);
	//RULE(BF_EQ_SIMPLIFY_1, "0 = 0 ::= T.")
	if (arg1(ch).is(tau::bf_f) && arg2(ch).is(tau::bf_f)) return _T(v, ch, len, r);
	//RULE(BF_DEF_SIMPLIFY_0, "0 = 1")
	if (arg1(ch).is(tau::bf_f) && arg2(ch).is(tau::bf_t)) return tau::get(tau::_F(), r);
	...
	if (!arg2(ch).is(tau::bf_f)) return tau::get(tau::build_bf_eq(
		tau::build_bf_xor(arg1_fm(ch).get(), arg2_fm(ch).get())), r);
	return tau::get_raw(v, ch, len, r);
}
...
tref get_hook<node>::wff_neq(const node& v, const tref* ch, size_t len, tref r) {
	...
	if (!arg2(ch).is(tau::bf_f)) return tau::get(tau::build_bf_neq(
		tau::build_bf_xor(arg1_fm(ch).get(), arg2_fm(ch).get())), r);
	return tau::get_raw(v, ch, len, r);
}
...
tref get_hook<node>::wff_conditional(const node& v, const tref* ch, size_t len, tref r) {
	...
	if (arg1(ch).is(tau::wff_f)) return tau::get(arg3_fm(ch).get(), r);
	//RULE(WFF_CONDITIONAL_SIMPLIFY_1, "T ? $X : $Y ::= $X.")
	if (arg1(ch).is(tau::wff_t)) return tau::get(arg2_fm(ch).get(), r);
	//RULE(WFF_CONDITIONAL_SIMPLIFY_4, "$X ? $Y : $Y ::= $Y.")
	if (arg2_fm(ch) == arg3_fm(ch)) return tau::get(arg3_fm(ch).get(), r);
	return tau::get_raw(v, ch, len, r);
}
	...
	[[maybe_unused]] size_t len, [[maybe_unused]] tref r)
{
	...
	if (arg2(ch).is(tau::wff_f)) return tau::get(tau::build_wff_neg(arg1_fm(ch).get()), r);
	//RULE(WFF_IMPLY_SIMPLIFY_3, "$X -> T ::= T.")
	if (arg2(ch).is(tau::wff_t)) return tau::get(tau::_T(), r);
	//RULE(WFF_IMPLY_SIMPLIFY_4, "$X -> $X ::= T.")
	if (arg1_fm(ch) == arg2_fm(ch)) return tau::get(tau::_T(), r);
	...
	[[maybe_unused]] size_t len, [[maybe_unused]] tref r)
{
	...
	if (arg2(ch).is(tau::wff_f)) return tau::get(tau::_T(), r);
	//RULE(WFF_RIMPLY_SIMPLIFY_1, "T <- $X ::= $X.")
	if (arg2(ch).is(tau::wff_t)) return tau::get(arg1_fm(ch).get(), r);
	//RULE(WFF_RIMPLY_SIMPLIFY_2, "$X <- F ::= ! $X.")
	if (arg1(ch).is(tau::wff_f)) return tau::get(tau::build_wff_neg(arg2_fm(ch).get()), r);
	...
	[[maybe_unused]] size_t len, [[maybe_unused]] tref r)
{
	...
	if (auto negated = arg2_fm(ch)() | tau::wff_neg | tau::wff;
		negated && negated.value() == arg1_fm(ch)) return tau::get(tau::_F(), r);
	//RULE(WFF_EQUIV_SIMPLIFY_6, "! $X <-> $X ::= F.")
	if (auto negated = arg1_fm(ch)() | tau::wff_neg | tau::wff;
		negated && negated.value() == arg2_fm(ch)) return tau::get(tau::_F(), r);
