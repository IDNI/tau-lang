...
#ifndef __SATISFIABILITY_DEPRECIATING_H__
#define __SATISFIABILITY_DEPRECIATING_H__
...
tau_<BAs...> build_io_out (const std::string& name, const std::string& var) {
	using p = tau_parser;
...
template<typename... BAs>
tau_<BAs...> build_io_in (const std::string& name, const std::string& var) {
	using p = tau_parser;
...
template<typename... BAs>
tau_<BAs...> build_io_out_const (const std::string& name, const int_t pos) {
	using p = tau_parser;
	auto var_name = wrap<BAs...>(p::out_var_name, name);
...
template<typename... BAs>
tau_<BAs...> build_io_in_const (const std::string& name, const int_t pos) {
	using p = tau_parser;
	auto var_name = wrap<BAs...>(p::in_var_name, name);
	auto offset = wrap<BAs...>(p::offset, build_int<BAs...>(pos));
	return wrap(p::variable, wrap(p::io_var, wrap(p::in, { var_name, offset })));
...
template<typename... BAs>
tau_<BAs...> build_io_out_shift (const std::string& name, const std::string& var, const int_t shift) {
	using p = tau_parser;
	auto var_name = wrap<BAs...>(p::out_var_name, name);
...
template<typename... BAs>
tau_<BAs...> build_io_in_shift (const std::string& name, const std::string& var, const int_t shift) {
	using p = tau_parser;
	auto var_name = wrap<BAs...>(p::in_var_name, name);
	auto shift_node = wrap<BAs...>(p::shift, {wrap<BAs...>(p::variable, var), build_num<BAs...>(shift)});
	auto offset = wrap<BAs...>(p::offset, shift_node);
...
template<typename... BAs>
bool has_stream_flag(const tau_<BAs...>& fm) {
	using p = tau_parser;
...
template<typename... BAs>
bool has_temporary_io_var (const tau_<BAs...>& fm) {
	using p = tau_parser;
...
template<typename... BAs>
tau_<BAs...> transform_io_var(const tau_<BAs...>& io_var, const std::string& io_var_name, int_t time_point) {
...
template<typename... BAs>
tau_<BAs...> existentially_quantify_output_streams(tau_<BAs...> fm, const auto& io_vars,
	...
	for (const auto& pos : quantifiable_o_vars) {
		...
		if (initials.contains({get_io_name(io_vars[pos]), time_point}))
			continue;
		tau_<BAs...> var = build_io_out_const<BAs...>(get_io_name(io_vars[pos]), time_point);
...
template<typename... BAs>
tau_<BAs...> universally_quantify_input_streams(tau_<BAs...> fm, const auto& io_vars,
                                               int_t time_point, const auto& initials) {
	// This map is needed in order to get the minimal shift for streams with same name
	...
	for (const auto& pos : quantifiable_i_vars) {
		...
		if (initials.contains({get_io_name(io_vars[pos]), time_point}))
			continue;
		tau_<BAs...> var = build_io_in_const<BAs...>(get_io_name(io_vars[pos]), time_point);
		auto res = cache.emplace(var);
...
template<typename... BAs>
tau_<BAs...> calculate_ctn(const tau_<BAs...>& constraint, int_t time_point) {
...
template<typename... BAs>
bool is_initial_ctn_phase(const tau_<BAs...>& constraint, int_t time_point) {
...
template<typename... BAs>
tau_<BAs...> fm_at_time_point(const tau_<BAs...>& original_fm,
...
template<typename... BAs>
std::pair<tau_<BAs...>, tau_<BAs...> > build_initial_step_chi(
	const tau_<BAs...>& chi, const tau_<BAs...>& st, auto& io_vars,
	...
	tau_<BAs...> c_pholder = build_io_out_const<BAs...>("_pholder", time_point);
	c_pholder = build_wff_eq(wrap(tau_parser::bf, c_pholder));
	pholder_to_st.emplace(c_pholder, replace(st, changes));
	tau_<BAs...> new_fm = build_wff_and(replace(chi, changes), c_pholder);
	return make_pair(new_fm, c_pholder);
}

template<typename... BAs>
tau_<BAs...> build_step(const tau_<BAs...>& original_fm,
	const tau_<BAs...>& prev_fm, const auto& io_vars, auto& initials, int_t step_num,
	int_t time_point, tau_<BAs...>& cached_fm) {
	...
	tau_<BAs...> most_inner_step = replace(original_fm, changes);
...
template<typename... BAs>
tau_<BAs...> build_step_chi(const tau_<BAs...>& chi, const tau_<BAs...>& st,
	const tau_<BAs...>& prev_fm, const auto& io_vars, auto& initials, int_t step_num,
	...
	tau_<BAs...> c_chi = replace(chi, changes);
	tau_<BAs...> c_pholder = build_io_out_const<BAs...>("_pholder", time_point + step_num);
	c_pholder = build_wff_eq(wrap(tau_parser::bf, c_pholder));
	tau_<BAs...> c_st = replace(st, changes);
...
template<typename... BAs>
tau_<BAs...> get_uninterpreted_constants_constraints(const tau_<BAs...>& fm, auto& io_vars) {
	using p = tau_parser;
...
template<typename... BAs>
std::pair<tau_<BAs...>, int_t> find_fixpoint_phi(const tau_<BAs...>& base_fm, const tau_<BAs...>& ctn_initials,
	const auto& io_vars, const auto& initials, const int_t time_point) {
	tau_<BAs...> phi_prev = fm_at_time_point(base_fm, io_vars, time_point);
	phi_prev = build_wff_and(ctn_initials, phi_prev);
	int_t step_num = 1;
	tau_<BAs...> cache = phi_prev;
...
template<typename... BAs>
std::pair<tau_<BAs...>, int_t> find_fixpoint_chi(const tau_<BAs...>& chi_base, const tau_<BAs...>& st,
			      const auto& io_vars, const auto& initials,
			      const int_t time_point) {
	std::map<tau_<BAs...>, tau_<BAs...>> pholder_to_st;
	...
	tau_<BAs...> chi = build_step_chi(chi_base, st, chi_prev, io_vars,
...
template<typename... BAs>
tau_<BAs...> transform_back_non_initials(const tau_<BAs...>& fm, const int_t highest_init_cond) {
	// Find lookback
...
template<typename... BAs>
tau_<BAs...> build_flag_on_lookback (const std::string& name, const std::string& var,
...
template<typename... BAs>
tau_<BAs...> build_prev_flag_on_lookback (const std::string& name, const std::string& var, const int_t lookback) {
	if (lookback >= 2)
		return wrap(tau_parser::bf,
...
template<typename... BAs>
tau_<BAs...> transform_ctn_to_streams(const tau_<BAs...>& fm, tau_<BAs...>& flag_initials,
					tau_<BAs...>& flag_rules,
					const int_t lookback,
	...
	auto create_initial = [&flag_initials](
		...
		tau_<BAs...> flag_init_cond = transform_io_var(
...
template<typename... BAs>
tau_<BAs...> always_to_unbounded_continuation(tau_<BAs...> fm,
	const int_t start_time,
	const bool output)
{
	...
	std::vector<tau_<BAs...> > io_vars = select_top(fm,
				is_child_non_terminal<p::io_var, BAs...>);
	int_t lookback = get_max_shift(io_vars);
	tau_<BAs...> flag_initials = _T<BAs...>, flag_rules = _T<BAs...>;
...
template<typename... BAs>
tau_<BAs...> create_guard(const auto& io_vars, const int_t number) {
	using p = tau_parser;
	tau_<BAs...> guard = _T<BAs...>;
...
template<typename... BAs>
std::pair<tau_<BAs...>, int_t> transform_to_eventual_variables(const tau_<BAs...>& fm, bool reset_ctn_stream, const int_t start_time) {
	using p = tau_parser;
	...
	int_t aw_lookback = 0;
	std::vector<tau_<BAs...>> aw_io_vars;
	if (aw_fm.has_value()) {
	...
	tau_<BAs...> ev_assm = _T<BAs...>;
	tau_<BAs...> ev_collection = _0<BAs...>;
	for (size_t n = 0; n < smt_fms.size(); ++n) {
		...
		tau_<BAs...> ctn_initials = _T<BAs...>, ctn_assm = _T<BAs...>;
...
template<typename... BAs>
tau_<BAs...> add_st_ctn (const tau_<BAs...>& st, const int_t timepoint, const int_t steps) {
	tau_<BAs...> st_ctn = _T<BAs...>;
...
template <typename... BAs>
tau_<BAs...> make_initial_run(const tau_<BAs...>& aw, const int_t max_st_lookback) {
	// get lookback of aw
	using p = tau_parser;
...
template<typename... BAs>
tau_<BAs...> to_unbounded_continuation(const tau_<BAs...>& ubd_aw_continuation,
				      const tau_<BAs...>& ev_var_flags,
	...
	std::vector<tau_<BAs...>> io_vars = select_top(aw,
			is_child_non_terminal<p::io_var, BAs...>);
	std::vector<tau_<BAs...>> st_io_vars = select_top(st_flags,
		is_child_non_terminal<p::io_var, BAs...>);
	...
	tau_<BAs...> run = make_initial_run(ori_aw_ctn, max_st_lookback);
...
template<typename... BAs>
tau_<BAs...> transform_to_execution(const tau_<BAs...>& fm,
...
template<typename... BAs>
bool is_tau_impl (const tau_<BAs...>& f1, const tau_<BAs...>& f2) {
...
template<typename... BAs>
bool are_tau_equivalent (const tau_<BAs...>& f1, const tau_<BAs...>& f2) {
