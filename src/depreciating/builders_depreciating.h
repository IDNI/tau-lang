...
#ifndef __BUILDERS_DEPRECIATING_H__
#define __BUILDERS_DEPRECIATING_H__
...
builder<BAs...> make_builder(const tau_<BAs...>& builder) {
...
template <typename... BAs>
tau_<BAs...> tau_apply_builder(const builder<BAs...>& b,
	std::vector<tau_<BAs...>>& n)
{
	std::map<tau_<BAs...>, tau_<BAs...>> changes;
	std::vector<tau_<BAs...>> vars = b.first || tau_parser::capture;
	for (size_t i = 0; i < vars.size(); ++i) changes[vars[i]] = n[i];
	return replace<tau_<BAs...>>(b.second, changes);
}
...
template <typename... BAs>
static const tau_<BAs...> _0 = bldr_bf_0<BAs...>.second;

template <typename... BAs>
static const tau_<BAs...> _0_trimmed = trim(_0<BAs...>);

template <typename... BAs>
static const tau_<BAs...> _1 = bldr_bf_1<BAs...>.second;

template <typename... BAs>
static const tau_<BAs...> _1_trimmed = trim(_1<BAs...>);

template <typename... BAs>
static const tau_<BAs...> _F = bldr_wff_F<BAs...>.second;

template <typename... BAs>
static const tau_<BAs...> _F_trimmed = trim(_F<BAs...>);

template <typename... BAs>
static const tau_<BAs...> _T = bldr_wff_T<BAs...>.second;

template <typename... BAs>
static const tau_<BAs...> _T_trimmed = trim(_T<BAs...>);

template <typename... BAs>
tau_<BAs...> build_num(size_t num) {
	return wrap(
...
template<typename... BAs>
tau_<BAs...> build_int(int_t i) {
	using p = tau_parser;
	std::vector<tau_<BAs...>> children;
	const bool n = i < 0;
...
template <typename... BAs>
tau_<BAs...> build_variable(const std::string& name) {
	return wrap<BAs...>(tau_parser::variable, name);
}

template <typename... BAs>
tau_<BAs...> build_in_var_name(const size_t& index) {
...
template<typename... BAs>
tau_<BAs...> build_type(const std::string& type) {
	return wrap<BAs...>(tau_parser::type, type);
}

template<typename... BAs>
tau_<BAs...> build_bf_t_type(const std::string& type) {
	return wrap(
		tau_parser::bf, wrap(
			tau_parser::bf_t,
				build_type<BAs...>(type)));
...
template<typename... BAs>
tau_<BAs...> build_bf_t_type(const tau_<BAs...>& type) {
	return wrap(
		tau_parser::bf, wrap(
			tau_parser::bf_t,
				type));
...
template<typename... BAs>
tau_<BAs...> build_bf_f_type(const std::string& type) {
	return wrap(
		tau_parser::bf, wrap(
			tau_parser::bf_f,
				build_type<BAs...>(type)));
...
template<typename... BAs>
tau_<BAs...> build_bf_f_type(const tau_<BAs...>& type) {
	return wrap(
		tau_parser::bf, wrap(
			tau_parser::bf_f,
				type));
...
template<typename... BAs>
tau_<BAs...> build_in_variable_at_n(const tau_<BAs...>& in_var_name, const size_t& num) {
...
template <typename... BAs>
tau_<BAs...> build_in_variable_at_n(const size_t& index, const size_t& num) {
	return build_in_variable_at_n(build_in_var_name<BAs...>(index), num);
}

template<typename... BAs>
tau_<BAs...> build_in_variable_at_n(const std::string& name, const int_t pos) {
	using p = tau_parser;
...
template <typename... BAs>
tau_<BAs...> build_in_variable_at_t(const tau_<BAs...>& in_var_name) {
	assert(is_non_terminal(tau_parser::in_var_name, in_var_name));
...
template <typename... BAs>
tau_<BAs...> build_in_variable_at_t(const size_t& index) {
	return build_in_variable_at_t(build_in_var_name<BAs...>(index));
}

template <typename... BAs>
tau_<BAs...> build_in_variable_at_t_minus (const std::string& name, const int_t shift) {
	using p = tau_parser;
	auto var_name = wrap<BAs...>(p::in_var_name, name);
...
template <typename... BAs>
tau_<BAs...> build_in_variable_at_t_minus(const tau_<BAs...>& in_var_name, const size_t& num) {
	assert(is_non_terminal(tau_parser::in_var_name, in_var_name));
	assert(num > 0);
...
template <typename... BAs>
tau_<BAs...> build_in_variable_at_t_minus(const size_t& index, const size_t& num) {
	return build_in_variable_at_t_minus(build_in_var_name<BAs...>(index), num);
}

template <typename... BAs>
tau_<BAs...> build_out_var_name(const size_t& index) {
	std::stringstream var_name;
	var_name << "o" << index;
...
template <typename... BAs>
tau_<BAs...> build_out_var_name(const std::string& name) {
	return wrap<BAs...>(tau_parser::out_var_name, name);
}

template <typename... BAs>
tau_<BAs...> build_out_variable_at_t(const tau_<BAs...>& out_var_name) {
...
template <typename... BAs>
tau_<BAs...> build_out_variable_at_t(const size_t& index) {
	return build_out_variable_at_t(build_out_var_name<BAs...>(index));
}

template <typename... BAs>
tau_<BAs...> build_out_variable_at_n(const tau_<BAs...>& out_var_name, const size_t& num) {
	assert(is_non_terminal(tau_parser::out_var_name, out_var_name));
...
template <typename... BAs>
tau_<BAs...> build_out_variable_at_n(const size_t& index, const size_t& num) {
	return build_out_variable_at_n(build_out_var_name<BAs...>(index), num);
}

template<typename... BAs>
tau_<BAs...> build_out_variable_at_n(const std::string& name, const int_t pos) {
	using p = tau_parser;
...
template <typename... BAs>
tau_<BAs...> build_out_variable_at_t_minus (const std::string& name, const int_t shift) {
	using p = tau_parser;
	auto var_name = wrap<BAs...>(p::out_var_name, name);
	auto shift_node = wrap<BAs...>(p::shift, {wrap<BAs...>(p::variable, "t"), build_num<BAs...>(shift)});
	auto offset = wrap<BAs...>(p::offset, shift_node);
...
template <typename... BAs>
tau_<BAs...> build_out_variable_at_t_minus(const tau_<BAs...>& out_var_name, const size_t& num) {
	assert(is_non_terminal(tau_parser::out_var_name, out_var_name));
	assert(num > 0);
...
template <typename... BAs>
int_t get_max_initial(const std::vector<tau_<BAs...>>& io_vars) {
	int_t max_init = -1;
	for (const tau_<BAs...>& v : io_vars) {
		if (is_io_initial(v)) {
...
template <typename... BAs>
tau_<BAs...> build_bf_var(const std::string& name) {
	auto var = make_builder<BAs...>("( $X ) =: " + name + ".").second;
	return trim<BAs...>(var);
}

template <typename... BAs>
tau_<BAs...> build_wff_var(const std::string& name) {
	auto var = make_builder<BAs...>("( $X ) =:: ?" + name + ".").second;
	return trim<BAs...>(var);
}

template <typename... BAs>
tau_<BAs...> build_bf_constant(const tau_<BAs...>& cte) {
	return wrap(
		tau_parser::bf, wrap(
...
template<typename... BAs>
tau_<BAs...> build_bf_constant(const tau_<BAs...>& cte, const tau_<BAs...>& type) {
	return wrap(
		tau_parser::bf, wrap(
			tau_parser::bf_constant, wrap(
				tau_parser::constant,
...
template<typename... BAs>
tau_<BAs...> build_bf_constant(const tau_<BAs...>& cte, const std::string& type) {
...
template <typename... BAs>
tau_<BAs...> build_bf_constant(const std::variant<BAs...>& v) {
	auto cte = rewriter::depreciating::make_node<tau_sym<BAs...>>(tau_sym<BAs...>(v), {});
	return build_bf_constant<BAs...>(cte);
}

template<typename... BAs>
tau_<BAs...> build_bf_constant(const std::variant<BAs...>& v, const tau_<BAs...>& type) {
	auto cte = rewriter::depreciating::make_node<tau_sym<BAs...>>(tau_sym<BAs...>(v), {});
	return build_bf_constant<BAs...>(cte, type);
}

template<typename... BAs>
tau_<BAs...> build_bf_constant(const std::variant<BAs...>& v, const std::optional<tau_<BAs...>>& type) {
	auto cte = rewriter::depreciating::make_node<tau_sym<BAs...>>(tau_sym<BAs...>(v), {});
	return type.has_value() ? build_bf_constant<BAs...>(cte, type.value()) : build_bf_constant<BAs...>(cte);
}

template<typename... BAs>
tau_<BAs...> build_bf_and_constant( const auto& ctes, const tau_<BAs...>& type)
{
...
template <typename... BAs>
tau_<BAs...> build_bf_or_constant( const auto& ctes, const tau_<BAs...>& type)
{
...
template <typename... BAs>
std::optional<tau_<BAs...>> build_bf_constant(
...
template <typename... BAs>
tau_<BAs...> build_bf_uniter_const(const std::string& n1, const std::string& n2) {
...
template <typename... BAs>
tau_<BAs...> build_wff_eq(const tau_<BAs...>& l) {
	return wrap(tau_parser::wff, wrap(tau_parser::bf_eq, l, _0<BAs...>));
}

template <typename... BAs>
tau_<BAs...> build_wff_neq(const tau_<BAs...>& l) {
	return wrap(tau_parser::wff, wrap(tau_parser::bf_neq, l, _0<BAs...>));
}

template <typename... BAs>
tau_<BAs...> build_wff_and(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
...
template <typename... BAs>
tau_<BAs...> build_wff_xor(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
	return build_wff_or(
...
template <typename... BAs>
tau_<BAs...> build_wff_imply(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
...
template <typename... BAs>
tau_<BAs...> build_wff_conditional(const tau_<BAs...>& x,
	const tau_<BAs...>& y,
	const tau_<BAs...>& z)
{
...
template <typename... BAs>
tau_<BAs...> build_wff_equiv(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
...
template <typename... BAs>
tau_<BAs...> build_wff_all(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
...
template <typename... BAs>
tau_<BAs...> build_wff_ex(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
...
template <typename... BAs>
tau_<BAs...> build_bf_xor(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
	return build_bf_or<BAs...>(
...
template<typename... BAs>
tau_<BAs...> build_wff_eq(const tau_<BAs...>& l, const tau_<BAs...>& r) {
#ifdef DEBUG
	using p = tau_parser;
...
template <typename... BAs>
tau_<BAs...> build_bf_less(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
...
template <typename... BAs>
tau_<BAs...> build_bf_nless(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
...
template <typename... BAs>
tau_<BAs...> build_bf_less_equal(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
...
template <typename... BAs>
tau_<BAs...> build_bf_nleq(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
...
template <typename... BAs>
tau_<BAs...> build_bf_interval(const tau_<BAs...>& x,
	const tau_<BAs...>& y, const tau_<BAs...>& z)
{
...
template <typename... BAs>
tau_<BAs...> build_bf_nleq_lower(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
	std::vector<tau_<BAs...>> args {trim(l), trim(r)};
	return tau_apply_builder<BAs...>(bldr_bf_nleq_lowwer<BAs...>, args);
}

template <typename... BAs>
tau_<BAs...> build_bf_nleq_upper(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
	std::vector<tau_<BAs...>> args {trim(l), trim(r)};
...
template <typename... BAs>
tau_<BAs...> build_bf_greater(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
...
template <typename... BAs>
tau_<BAs...> build_bf_ngreater(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
...
template <typename... BAs>
tau_<BAs...> build_bf_greater_equal(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
...
template <typename... BAs>
tau_<BAs...> build_bf_ngeq(const tau_<BAs...>& l,
	const tau_<BAs...>& r)
{
...
template <typename... BAs>
tau_<BAs...> build_wff_ctn_greater_equal(const tau_<BAs...>& ctnvar,
	const tau_<BAs...>& num) {
...
template <typename... BAs>
tau_<BAs...> build_wff_ctn_greater(const tau_<BAs...>& ctnvar,
	const tau_<BAs...>& num) {
	return wrap(tau_parser::wff,
			wrap(tau_parser::constraint,
...
template <typename... BAs>
tau_<BAs...> build_wff_ctn_less_equal(const tau_<BAs...>& ctnvar,
	const tau_<BAs...>& num) {
	return wrap(tau_parser::wff,
			wrap(tau_parser::constraint,
...
template <typename... BAs>
tau_<BAs...> build_wff_ctn_less(const tau_<BAs...>& ctnvar,
	const tau_<BAs...>& num) {
	return wrap(tau_parser::wff,
			wrap(tau_parser::constraint,
...
template <typename... BAs>
tau_<BAs...> build_wff_ctn_eq(const tau_<BAs...>& ctnvar,
	const tau_<BAs...>& num) {
	return wrap(tau_parser::wff,
			wrap(tau_parser::constraint,
...
template <typename... BAs>
tau_<BAs...> build_wff_ctn_neq(const tau_<BAs...>& ctnvar,
	const tau_<BAs...>& num) {
	return wrap(tau_parser::wff,
			wrap(tau_parser::constraint,
