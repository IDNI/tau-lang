...
template <typename node_t>
std::ostream& print_tau_source_node_tree(std::ostream &os,
	rewriter::depreciating::node<node_t> n, size_t l = 0)
{
	...
	if (n.child.size()) os << " {\n";
	for (auto& c : n.child) print_tau_source_node_tree<node_t>(os, *c, l + 1);
