...
#ifndef __INTERPRETER_DEPRECIATING_H__
#define __INTERPRETER_DEPRECIATING_H__
...
using io_var_name = tau_<BAs...>; // out_var_name or in_var_name
...
struct finputs {
	...
	finputs(std::map<tau_<BAs...>, std::pair<std::string, filename>> inputs) {
	...
	void add_input (const tau_<BAs...>& var, const std::string& t, const filename& f) {
	...
	std::pair<std::optional<assignment<BAs...> >, bool> read(
		std::vector<tau_<BAs...> >& in_vars, size_t time_step) {
	...
	std::optional<std::string> type_of(const tau_<BAs...>& var) const {
	...
	std::map<tau_<BAs...>, std::string> types;
	std::map<tau_<BAs...>, std::optional<std::ifstream>> streams;
	size_t time_point = 0;
};
...
struct foutputs {
	...
	foutputs(std::map<tau_<BAs...>, std::pair<std::string, filename>> outputs) {
		// open the corresponding streams for input and store them in streams
	...
	void add_output (const tau_<BAs...>& var, const std::string& t, const filename& f) {
		if (!types.contains(var)) {
			types.emplace(var, t);
			streams.emplace(var,
	...
	std::map<tau_<BAs...>, std::string> types;
	std::map<tau_<BAs...>, std::optional<std::ofstream>> streams;
};

template<typename input_t, typename output_t, typename...BAs>
struct interpreter {
	interpreter(const tau_<BAs...>& ubt_ctn, const tau_<BAs...>& original_spec,
	...
	static std::optional<interpreter> make_interpreter(
		const tau_<BAs...>& spec, auto& inputs, auto& outputs);

	std::pair<std::optional<assignment<BAs...>>, bool> step();

	// Update the interpreter with a given update
	void update(const tau_<BAs...>& update);

	tau_<BAs...> ubt_ctn;
	tau_<BAs...> original_spec;
...
private:
	...
	tau_<BAs...> get_ubt_ctn_at (int_t t);

	bool calculate_initial_systems ();

	std::vector<tau_<BAs...>> build_inputs_for_step (const size_t t);

	// Return typed systems of equations for the solver corresponding to each clause
	// in the unbound continuation
	static std::vector<system<BAs...>> compute_systems(const tau_<BAs...>& ubd_ctn,
		auto& inputs, auto& outputs);

	// Get the type for a clause of a local specification
	static std::optional<system<BAs...>> compute_atomic_fm_types(const tau_<BAs...>& clause,
		auto& inputs, auto& outputs);
	...
	static std::optional<std::string> get_type_atomic_fm(const tau_<BAs...>& fm,
		auto& inputs, auto& outputs);

	tau_<BAs...> update_to_time_point(const tau_<BAs...>& f, const int_t t);
	...
	static std::pair<tau_<BAs...>, tau_<BAs...>>
	get_executable_spec(const tau_<BAs...>& fm, const size_t start_time = 0);

	// Pointwise revision algorithm for producing updated specification
	// Both spec and update need to be normalized
	tau_<BAs...> pointwise_revision(tau_<BAs...> spec, tau_<BAs...> update,
	...
	solution_with_max_update(const tau_<BAs...>& spec);

	// Returns if the variable is excluded from output
	static bool is_excluded_output(const tau_<BAs...>& var);

	// Return those variables that appear within the lookback
	std::vector<tau_<BAs...> > appear_within_lookback(
		const std::vector<tau_<BAs...> >& vars);

};

template<typename... BAs>
std::optional<tau_<BAs...>> unpack_tau_constant(const tau_<BAs...>& constant) {
	using p = tau_parser;
...
run(const tau_<BAs...>& form, input_t& inputs, output_t& outputs, const size_t steps = 0) {
...
# endif //__INTERPRETER_DEPRECIATING_H__