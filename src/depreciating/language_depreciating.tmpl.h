...
{
	return wrap(nt, std::vector<tau_<BAs...>>(ch));
}
...
auto get_optional(const extractor_t& extractor, const tau_<BAs...>& n) {
...
template <size_t... nt, typename extractor_t, typename... BAs>
auto get_optionals(const extractor_t& extractor, const tau_<BAs...>& n) {
...
// bind the given, using a binder, the constants of the a given tau_<...>.
...
template <typename... BAs>
static const auto not_whitespace_predicate = [](const tau_<BAs...>& n) {
...
struct name_binder {
	...
	tau_<BAs...> bind(const tau_<BAs...>& n) {
		...
		if (auto s = bs.find(bn); s != bs.end()) {
			...
			return wrap(tau_parser::bf_constant,
				wrap(tau_parser::constant,
					rewriter::depreciating::make_node<tau_sym<BAs...>>(
								ts, {})));
		}
		return error = true, n;
	}
...
template <typename... BAs>
rewriter::depreciating::rule<tau_<BAs...>> make_rule(tau_parser::nonterminal rule_t,
...
// TODO (LOW) should depend in node_t instead of BAs...
template <typename... BAs>
rec_relations<tau_<BAs...>> make_rec_relations(
	const tau_<BAs...>& tau_source)
{
	rec_relations<tau_<BAs...>> rs;
	// TODO (LOW) change call to select by operator|| and operator|
	for (auto& r: select_top(tau_source,
...
template <typename...BAs>
tau_<BAs...> process_digits(const tau_<BAs...>& tau_source) {
	std::map<tau_<BAs...>, tau_<BAs...>> changes;
		...
		is_non_terminal<tau_parser::digits, BAs...>))
	{
		...
		auto nn = rewriter::depreciating::make_node<tau_sym<BAs...>>(tau_sym<BAs...>(num), {});
		changes[n] = nn;
	}
	return replace<tau_<BAs...>>(tau_source, changes);
}
...
template <typename...BAs>
tau_<BAs...> process_quantifier_vars(const tau_<BAs...>& tau_code) {
	using node = tau_<BAs...>;
	quantifier_vars_transformer<BAs...> transformer;
	return rewriter::depreciating::post_order_traverser<quantifier_vars_transformer<BAs...>,
		rewriter::depreciating::all_t, node>(transformer, rewriter::depreciating::all)(tau_code);
}

template <typename...BAs>
tau_<BAs...> process_offset_variables(const tau_<BAs...>& tau_code) {
	using p = tau_parser;
	using node = tau_<BAs...>;
	std::map<node, node> changes;
...
template <typename...BAs>
tau_<BAs...> process_defs_input_variables(const tau_<BAs...>& tau_code) {
	using p = tau_parser;
	using node = tau_<BAs...>;
	std::map<node, node> changes;
	for (const auto& def :
...
template <typename... BAs>
tau_<BAs...> infer_constant_types(const tau_<BAs...>& code) {
	...
	auto infer_over_scope = [&](const node& nd) {
				...
				tau_parser::bf_constant, BAs...>))
		{
			...
			} else changes.emplace(c, rewriter::depreciating::make_node<
...
struct free_vars_collector {
	...
	free_vars_collector(std::set<tau_<BAs...>>& free_vars) : free_vars(free_vars) {}
	...
	std::set<tau_<BAs...>>& free_vars;
};

template <typename... BAs>
auto get_free_vars_from_nso(const tau_<BAs...>& n) {
	BOOST_LOG_TRIVIAL(trace) << "(I) -- Begin get_free_vars_from_nso of " << n;
	std::set<tau_<BAs...>> free_vars;
	free_vars_collector<BAs...> collector(free_vars);
	rewriter::depreciating::post_order_traverser<
			free_vars_collector<BAs...>,
			rewriter::depreciating::all_t,
			tau_<BAs...>>(collector, rewriter::depreciating::all)(n);
...
template<typename ...BAs>
void get_leaves(const tau_<BAs...>& n, tau_parser::nonterminal branch,
	std::vector<tau_<BAs...>>& leaves)
{
	...
	rewriter::depreciating::pre_order(n).visit(add_leave);
}

template<typename ...BAs>
std::vector<tau_<BAs...>> get_leaves(const tau_<BAs...>& n,
...
template<typename ...BAs>
std::vector<tau_<BAs...>> get_dnf_wff_clauses(const tau_<BAs...>& n) {
	return get_leaves(n, tau_parser::wff_or);
}

template<typename ...BAs>
std::vector<tau_<BAs...>> get_dnf_bf_clauses(const tau_<BAs...>& n) {
	return get_leaves(n, tau_parser::bf_or);
}

template<typename ...BAs>
std::vector<tau_<BAs...>> get_cnf_wff_clauses(const tau_<BAs...>& n) {
	return get_leaves(n, tau_parser::wff_and);
}

template<typename ...BAs>
std::vector<tau_<BAs...>> get_cnf_bf_clauses(const tau_<BAs...>& n) {
...
template <typename... BAs>
bool has_temp_var (const tau_<BAs...>& fm) {
...
template <typename... BAs>
bool invalid_nesting_of_quants (const tau_<BAs...>& fm) {
...
template <typename... BAs>
bool invalid_nesting_of_temp_quants (const tau_<BAs...>& fm) {
...
template <typename... BAs>
bool has_open_tau_fm_in_constant (const tau_<BAs...>& fm) {
...
template <typename... BAs>
bool has_negative_offset(const tau_<BAs...>& fm) {
	using p = tau_parser;
...
template <typename... BAs>
bool has_semantic_error (const tau_<BAs...>& fm) {
...
tau_<BAs...> make_tau_code(sp_tau_source_node& tau_source) {
	...
	tauify<BAs...> tf;
	rewriter::depreciating::map_transformer<tauify<BAs...>,
		sp_tau_source_node, tau_<BAs...>> transform(tf);
	auto tau_code = rewriter::depreciating::post_order_traverser<
				rewriter::depreciating::map_transformer<tauify<BAs...>,
				sp_tau_source_node, tau_<BAs...>>,
			rewriter::depreciating::all_t,
			rewriter::depreciating::sp_node<tau_source_sym>,
			tau_<BAs...>>(
		transform, rewriter::depreciating::all)(tau_source);
...
template<typename binder_t, typename... BAs>
tau_<BAs...> bind_tau_code_using_binder(const tau_<BAs...>& code,
	binder_t& binder) {
	bind_transformer<binder_t, BAs...> bs(binder);
	auto res = rewriter::depreciating::post_order_traverser<
			bind_transformer<binder_t, BAs...>,
			rewriter::depreciating::all_t,
			tau_<BAs...>>(bs, rewriter::depreciating::all)(code);
	...
	const tau_<BAs...>& code)
{
	...
	auto main = (is_non_terminal(tau_parser::tau_constant_source, code)
		|| is_non_terminal(tau_parser::spec, code)
			? code | tau_parser::main
			: code | tau_parser::spec | tau_parser::main)
				| tau_parser::wff
				| optional_value_extractor<tau_<BAs...>>;
	auto rules = make_rec_relations<BAs...>(code);
	return infer_ref_types<BAs...>(rr<tau_<BAs...>>{ rules, main });
}

// make a nso_rr from the given tau source and binder.
template<typename binder_t, typename... BAs>
std::optional<rr<tau_<BAs...>>> make_nso_rr_using_binder(const tau_<BAs...>& code,
	binder_t& binder)
{
...
template<typename binder_t, typename... BAs>
tau_<BAs...> make_nso_using_binder(const tau_<BAs...>& code,
	binder_t& binder)
{
...
// make a nso_rr from the given tau source and binder.
template<typename binder_t, typename... BAs>
std::optional<rr<tau_<BAs...>>> make_nso_rr_using_binder(
...
template<typename binder_t, typename... BAs>
tau_<BAs...> make_nso_using_binder(sp_tau_source_node& source,
	binder_t& binder)
{
	auto code = make_tau_code<BAs...>(source);
...
// make a nso_rr from the given tau source and binder.
template<typename binder_t, typename... BAs>
std::optional<rr<tau_<BAs...>>> make_nso_rr_using_binder(std::string& input,
	binder_t& binder)
{
	auto source = make_tau_source(input, { .start = tau_parser::spec });
	return !source ? std::optional<rr<tau_<BAs...>>>{}
		: std::optional<rr<tau_<BAs...>>>{
			make_nso_rr_using_binder<binder_t, BAs...>(
				source, binder) };
}

template<typename binder_t, typename... BAs>
std::optional<tau_<BAs...>> make_nso_using_binder(std::string& input,
	binder_t& binder,
	idni::parser<>::parse_options options = { .start = tau_parser::wff })
{
	auto source = make_tau_source(input, options);
	return !source ? std::optional<tau_<BAs...>>{}
...
std::optional<rr<tau_<BAs...>>> make_nso_rr_using_bindings(
	const tau_<BAs...>& code, const bindings<BAs...>& bindings)
{
	name_binder<BAs...> nb(bindings);
...
{
	auto source = make_tau_source(input, { .start = tau_parser::spec });
	if (!source) return std::optional<rr<tau_<BAs...>>>{};
	return make_nso_rr_using_bindings<BAs...>(source, bindings);
}
// make a nso from the given tau source and bindings.
template <typename... BAs>
std::optional<tau_<BAs...>> make_nso_using_bindings(
	const std::string& input, const bindings<BAs...>& bindings,
	idni::parser<>::parse_options options)
{
	auto source = make_tau_source(input, options);
	if (!source) return std::optional<tau_<BAs...>>{};
	return make_nso_using_bindings<BAs...>(source, bindings);
}

template <typename... BAs>
std::optional<rr<tau_<BAs...>>> make_nso_rr_using_factory(
	const tau_<BAs...>& code)
{
	factory_binder<BAs...> fb;
...
{
	auto code = make_tau_code<BAs...>(source);
	if (!code) return std::optional<rr<tau_<BAs...>>>{};
	return make_nso_rr_using_factory<BAs...>(code);
}

template <typename... BAs>
std::optional<tau_<BAs...>> make_nso_using_factory(
	sp_tau_source_node& source)
{
	auto code = make_tau_code<BAs...>(source);
	if (!code) return std::optional<tau_<BAs...>>{};
...
template <typename... BAs>
rr_sig get_rr_sig(const tau_<BAs...>& n) {
...
template <typename... BAs>
struct rr_types {
	friend std::optional<rr<tau_<BAs...>>> infer_ref_types<BAs...>(
		const rr<tau_<BAs...>>&, rr_types<BAs...>&);
	rr_types(const rr<tau_<BAs...>>& nso_rr) { get_ref_types(nso_rr); }
...
private:
	...
	bool add(const tau_<BAs...>& n, const tau_parser::nonterminal& t) {
		auto ref = n;
		if (auto ref_as_child = n | tau_parser::ref; ref_as_child)
			ref = ref_as_child.value();
	...
	bool get_types(const tau_<BAs...>& n, bool def = false) {
	...
	bool get_ref_types(const rr<tau_<BAs...>>& nso_rr) {
...
template <typename... BAs>
std::optional<rr<tau_<BAs...>>> infer_ref_types(const rr<tau_<BAs...>>& nso_rr,
	rr_types<BAs...>& ts)
{
