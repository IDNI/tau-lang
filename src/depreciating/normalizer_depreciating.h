...
#ifndef __NORMALIZER2_DEPRECIATING_H__
#define __NORMALIZER2_DEPRECIATING_H__
...
rr<tau_<BAs...>> normalizer(std::string& source, bindings<BAs...>& binds) {
...
rr<tau_<BAs...>> normalizer(std::string& source, factory_t& factory) {
	auto form_source = make_tau_source(source, { .start = tau_parser::spec });
...
template<typename... BAs>
auto get_vars_from_nso(const tau_<BAs...>& n) {
	return select_top(n, is_var_or_capture<BAs...>);
}

// Given a tau_<BAs...> produce a number such that the variable x_i is
// neither a bool_variable nor a variable nor a capture
template<typename... BAs>
int_t get_new_var_id (const tau_<BAs...> fm) {
...
// Given a tau_<BAs...> produce a number i such that the uninterpreted constant const_i is
// not present
template<typename... BAs>
tau_<BAs...> get_new_uniter_const (const tau_<BAs...> fm, const std::string& name) {
...
template <typename... BAs>
std::optional<tau_<BAs...>> get_ref(const tau_<BAs...>& n) {
	auto ref = std::optional<tau_<BAs...>>(n);
...
template<typename ...BAs>
bool has_no_boolean_combs_of_models(const tau_<BAs...>& fm) {
...
template<typename... BAs>
bool is_non_temp_nso_satisfiable (const tau_<BAs...>& fm) {
...
template<typename... BAs>
bool is_non_temp_nso_unsat (const tau_<BAs...>& fm) {
	assert(!find_top(fm, is_non_terminal<tau_parser::wff_always, BAs...>));
	assert(!find_top(fm, is_non_terminal<tau_parser::wff_sometimes, BAs...>));
...
template <typename... BAs>
bool are_nso_equivalent(tau_<BAs...> n1, tau_<BAs...> n2) {
	...
	tau_<BAs...> imp1 = build_wff_imply<BAs...>(n1, n2);
	tau_<BAs...> imp2 = build_wff_imply<BAs...>(n2, n1);
...
template <typename... BAs>
auto is_nso_equivalent_to_any_of(tau_<BAs...>& n, std::vector<tau_<BAs...>>& previous) {
...
template<typename... BAs>
bool is_nso_impl (tau_<BAs...> n1, tau_<BAs...> n2) {
	...
	tau_<BAs...> imp = build_wff_imply<BAs...>(n1, n2);
...
template<typename... BAs>
bool are_bf_equal(tau_<BAs...> n1, tau_<BAs...> n2) {
	...
	tau_<BAs...> bf_equal_fm = build_wff_eq(build_bf_xor(n1, n2));
...
template<typename... BAs>
tau_<BAs...> normalize_with_temp_simp (const tau_<BAs...>& fm) {
	using p = tau_parser;
	...
	for (const auto& clause : clauses) {
		...
		tau_<BAs...> new_clause = replace(clause, changes);
...
template <typename... BAs>
size_t get_max_loopback_in_rr(const tau_<BAs...>& form) {
	size_t max = 0;
...
template<typename... BAs>
tau_<BAs...> build_shift_from_shift(tau_<BAs...> shift, size_t step) {
	auto num = shift | tau_parser::num | optional_value_extractor<tau_<BAs...>>;
	auto offset = num | only_child_extractor<BAs...> | size_t_extractor<BAs...> | optional_value_extractor<size_t>;
	if (step == offset) return shift | tau_parser::capture | optional_value_extractor<tau_<BAs...>>;
	std::map<tau_<BAs...>, tau_<BAs...>> changes{{num, build_num<BAs...>(step - offset)}};
	return replace<tau_<BAs...>>(shift, changes);
}

template<typename... BAs>
tau_<BAs...> build_main_step(const tau_<BAs...>& form, size_t step) {
	std::map<tau_<BAs...>, tau_<BAs...>> changes;
	...
	return replace<tau_<BAs...>>(form, changes);
}

// Normalizes a Boolean function having no recurrence relation
template<typename... BAs>
tau_<BAs...> bf_normalizer_without_rec_relation (const tau_<BAs...>& bf) {
...
template<typename... BAs>
tau_<BAs...> bf_normalizer_with_rec_relation(const rr<tau_<BAs...>> &bf) {
...
template <typename... BAs>
tau_<BAs...> build_enumerated_main_step(const tau_<BAs...>& form, size_t i,
	size_t offset_arity)
{
	auto r = form;
	std::map<tau_<BAs...>, tau_<BAs...>> changes;
	std::vector<tau_<BAs...>> ofs; // create offsets node
...
template <typename... BAs>
bool is_valid(const rr<tau_<BAs...>>& nso_rr) {
...
template <typename... BAs>
bool is_well_founded(const rr<tau_<BAs...>>& nso_rr) {
...
template <typename... BAs>
tau_<BAs...> calculate_fixed_point(const rr<tau_<BAs...>>& nso_rr,
...
struct fixed_point_transformer {
	...
	fixed_point_transformer(const rr<tau_<BAs...>>& defs,
	...
	tau_<BAs...> get_fallback(tau_parser::nonterminal t,
		const tau_<BAs...>& ref)
	{
...
template<typename... BAs>
tau_<BAs...> calculate_all_fixed_points(const rr<tau_<BAs...>>& recrel) {
	...
	auto new_main = rewriter::depreciating::post_order_traverser<decltype(fpt),
		rewriter::depreciating::all_t, tau_<BAs...>>(fpt, rewriter::depreciating::all)(recrel.main);
...
template <typename... BAs>
tau_<BAs...> normalizer(const tau_<BAs...>& form) {
