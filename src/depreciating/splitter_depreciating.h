...
#ifndef __SPLITTER_DEPRECIATING_H__
#define __SPLITTER_DEPRECIATING_H__
...
tau_<BAs...> split(const tau_<BAs...>& fm, const size_t fm_type, bool is_cnf,
		   const splitter_type st, std::vector<tau_<BAs...> >& mem,
...
template<typename... BAs>
bool is_splitter(const tau_<BAs...>& fm, const tau_<BAs...>& splitter, const tau_<BAs...>& spec_clause = nullptr) {
	if (spec_clause) {
		// We are dealing with a temporal formula
		assert(is_tau_impl(splitter, fm));
		auto new_spec_clause = normalize_with_temp_simp(replace(spec_clause, fm, splitter));
...
template<typename... BAs>
tau_<BAs...> good_splitter_using_function(const tau_<BAs...>& f, splitter_type st,
					 const tau_<BAs...>& original_fm) {
	assert(is_non_terminal(tau_parser::bf, f));
	// First check if we have more then one disjunct
	std::vector<tau_<BAs...> > m;
	size_t i = 0;
	do {
		auto s = split(f, tau_parser::bf, false, st, m, i, true);
		if (s != f) {
			auto new_fm = replace(original_fm, f, s);
	...
	std::vector<tau_<BAs...> > clauses = get_leaves(f, tau_parser::bf_or);
	...
	for (const auto &clause: clauses) {
		...
		if (s != coeff.value()) {
			auto new_fm = replace(original_fm, coeff.value(), trim(s));
			if(!are_nso_equivalent(original_fm, new_fm))
				return new_fm;
		}
	}
...
template<typename... BAs>
tau_<BAs...> good_reverse_splitter_using_function(const tau_<BAs...> &f, splitter_type st,
						 const tau_<BAs...> &original_fm) {
	assert(is_non_terminal(tau_parser::bf, f));
	...
	std::vector<tau_<BAs...> > clauses = get_leaves(f_cnf, tau_parser::bf_and);
	for (const auto &clause: clauses) {
...
template<typename... BAs>
tau_<BAs...> tau_bad_splitter(tau_<BAs...> fm = _T<BAs...>) {
...
// We assume the formula is fully normalized by normalizer
template<typename... BAs>
std::pair<tau_<BAs...>, splitter_type> nso_tau_splitter(
	tau_<BAs...> fm, splitter_type st, const tau_<BAs...>& spec_clause = nullptr) {
	...
	for (const auto& clause: clauses) {
		...
		for (const auto &eq: eqs) {
			...
			std::set<tau_<BAs...>> free_vars = get_free_vars_from_nso(fm);
			...
			if (auto s = good_reverse_splitter_using_function(f, st, clause); s != clause) {
				...
				auto new_fm = replace(fm, clause, s);
				if (is_splitter(fm, new_fm, spec_clause))
					return {new_fm, st};
			}
		}
		...
		for (const auto &neq: neqs) {
			...
			for (const auto& c : bf_constants) {
				...
				auto new_fm = replace(fm, clause, build_wff_and(clause, r));
				new_fm = replace(new_fm, neq, _T_trimmed<BAs...>);
				if (is_splitter(fm, new_fm, spec_clause))
					return {new_fm, st};
			}
...
template<typename... BAs>
tau_<BAs...> tau_splitter (const tau_<BAs...>& fm, splitter_type st) {
	...
	auto splitter_of_clause = [&](const tau_<BAs...>& clause) {
	...
	if (clauses.size() == 1) {
		...
		if (aw.has_value()) {
			...
			return replace(fm, aw.value(), build_wff_always(aw_bad_splitter));
