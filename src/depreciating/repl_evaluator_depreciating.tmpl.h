	...
	const size_t nt, const tau_nso_t& program)
{
	...
	if (contains_ref && nt == tau_parser::spec) {
...
template<typename...BAs>
solver_mode get_solver_cmd_mode(const tau_<BAs...>& n) {
...
template<typename...BAs>
std::optional<std::string> get_solver_cmd_type(const tau_<BAs...>& n) {
...
void print_solver_cmd_solution(std::optional<solution<BAs...>>& solution,
	...
	auto print_zero_case = [&options](const tau_<BAs...>& var) {
		std::cout << "\t" << var << " := {"
			<< nso_factory<BAs...>::instance().zero(options.type)
			<< "}:" << options.type << "\n";
	};

	auto print_one_case = [&options](const tau_<BAs...>& var) {
		std::cout << "\t" << var << " := {"
...
template <typename... BAs>
tau_nso<BAs...> repl_evaluator<BAs...>::make_cli(
...
template <typename... BAs>
repl_option get_opt(const tau_<BAs...>& n) {
	auto o = n | tau_parser::option_name;
	if (!o) return none_opt;
	auto x = make_string<tau_node_terminal_extractor_t<BAs...>,tau_<BAs...>>(
