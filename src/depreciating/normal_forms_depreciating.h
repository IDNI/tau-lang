...
#ifndef __NORMAL_FORMS_DEPRECIATING_H__
#define __NORMAL_FORMS_DEPRECIATING_H__

#include <list>
#include <numeric>

#include "boolean_algebras/nso_ba_depreciating.h"
#include "execution_depreciating.h"
...
template <typename... BAs>
tau_<BAs...> to_mnf (const tau_<BAs...>& fm) {
	using p = tau_parser;
	auto neq_to_eq = [](const tau_<BAs...>& n) {
		//$X != 0 ::= !($X = 0)
	...
	return rewriter::depreciating::pre_order(fm).apply_unique(neq_to_eq,
		visit_wff<BAs...>, rewriter::depreciating::identity);
}

template <typename... BAs>
tau_<BAs...> from_mnf_to_nnf (const tau_<BAs...>& fm) {
	using p = tau_parser;
	auto ne_to_neq = [](const tau_<BAs...>& n) {
		//!($X = 0) ::= $X != 0
		if (is_non_terminal(p::wff_neg, n)) {
			const auto& e = trim2(n);
			if (is_non_terminal(p::bf_eq, e)) {
				return trim(build_wff_neq(trim(e)));
			}
		}
		return n;
	};
	return rewriter::depreciating::pre_order(fm).apply_unique(ne_to_neq,
		visit_wff<BAs...>, rewriter::depreciating::identity);
}
...
template <typename... BAs>
tau_<BAs...> unsqueeze_wff (const tau_<BAs...>& fm) {
	// $X | $Y = 0 ::= $X = 0 && $Y = 0
	// $X | $Y != 0 ::= $X != 0 || $Y != 0
	using p = tau_parser;
	auto f = [](const tau_<BAs...>& n) {
	...
	return rewriter::depreciating::pre_order(fm).apply_unique(f,
		visit_wff<BAs...>, rewriter::depreciating::identity);
}

template <typename... BAs>
tau_<BAs...> squeeze_wff (const tau_<BAs...>& fm) {
	//$X = 0 && $Y = 0 ::= $X | $Y = 0
	// $X != 0 || $Y != 0 ::= $X | $Y != 0
	using p = tau_parser;
	auto f = [](const tau_<BAs...>& n) {
	...
	return rewriter::depreciating::post_order(fm).apply_unique(f,
		visit_wff<BAs...>);
}

template <typename... BAs>
tau_<BAs...> unsqueeze_wff_pos (const tau_<BAs...>& fm) {
	// $X | $Y = 0 ::= $X = 0 && $Y = 0
	using p = tau_parser;
	auto f = [](const tau_<BAs...>& n) {
		if (is_non_terminal(p::bf_eq, n)) {
...
template <typename... BAs>
tau_<BAs...> squeeze_wff_pos (const tau_<BAs...>& fm) {
	// $X = 0 && $Y = 0 ::= $X | $Y = 0
	using p = tau_parser;
	auto f = [](const tau_<BAs...>& n) {
		if (is_non_terminal(p::wff_and, n)) {
...
template <typename... BAs>
tau_<BAs...> unsqueeze_wff_neg (const tau_<BAs...>& fm) {
	// $X | $Y != 0 ::= $X != 0 || $Y != 0
	using p = tau_parser;
	auto f = [](const tau_<BAs...>& n) {
		if (is_non_terminal(p::bf_neq, n)) {
...
template <typename... BAs>
tau_<BAs...> squeeze_wff_neg (const tau_<BAs...>& fm) {
	// $X != 0 || $Y != 0 ::= $X | $Y != 0
	using p = tau_parser;
	auto f = [](const tau_<BAs...>& n) {
		if (is_non_terminal(p::wff_or, n)) {
...
tau_<BAs...> to_dnf(const tau_<BAs...>&);
template<bool is_wff = true, typename... BAs>
tau_<BAs...> to_cnf(const tau_<BAs...>&);
template<bool is_wff = true, typename... BAs>
tau_<BAs...> push_negation_in(const tau_<BAs...>&);

template<typename... BAs>
tau_<BAs...> to_nnf (const tau_<BAs...>& fm) {
	return push_negation_in(fm);
}

// --------------------------------------------------------------
// General operator for tau_<BAs...> function application by pipe
template<typename... BAs>
using tau_f = tau_<BAs...>(*)(const tau_<BAs...>&);

template<typename... BAs>
tau_<BAs...> operator|(const tau_<BAs...>& fm, const tau_f<BAs...> func) {
	return func(fm);
}
// --------------------------------------------------------------
...
template <typename... BAs>
tau_<BAs...> normalize_ba(const tau_<BAs...>& fm) {
	...
	return rewriter::depreciating::pre_order(fm).template
...
struct reduce_deprecated {
	...
	tau_<BAs...> operator()(const tau_<BAs...>& form) const {
		std::map<tau_<BAs...>, tau_<BAs...>> changes;
...
private:
	...
	void get_literals(const tau_<BAs...>& clause, std::set<tau_<BAs...>>& literals) const {
	...
	std::set<tau_<BAs...>> get_literals(const tau_<BAs...>& clause) const {
		std::set<tau_<BAs...>> literals;
		get_literals(clause, literals);
		return literals;
	}

	std::pair<std::set<tau_<BAs...>>, std::set<tau_<BAs...>>> get_positive_negative_literals(const tau_<BAs...> clause) const {
		std::set<tau_<BAs...>> positives;
		std::set<tau_<BAs...>> negatives;
	...
	std::set<tau_<BAs...>> get_dnf_clauses(const tau_<BAs...>& n, std::set<tau_<BAs...>> clauses = {}) const {
		if constexpr (type == tau_parser::bf)
	...
	std::optional<tau_<BAs...>> build_dnf_clause_from_literals(const std::set<tau_<BAs...>>& positives, const std::set<tau_<BAs...>>& negatives) const {
		if (positives.empty() && negatives.empty()) {
			BOOST_LOG_TRIVIAL(debug) << "(F) {}";
			return {};
		}

		std::vector<tau_<BAs...>> literals;
	...
	std::optional<tau_<BAs...>> to_minterm(const tau_<BAs...>& clause) const {
	...
	tau_<BAs...> build_dnf_from_clauses(const std::set<tau_<BAs...>>& clauses) const {
		std::stringstream ss;
		if constexpr (type == tau_parser::bf) {
			if (clauses.empty()) {
				BOOST_LOG_TRIVIAL(debug) << "(F) " << SS(_0<BAs...>);
				return _0<BAs...>;
			}
		} else {
			if (clauses.empty()) {
				BOOST_LOG_TRIVIAL(debug) << "(F) " << SS(_F<BAs...>);
				return _F<BAs...>;
			}
		}
		...
		BOOST_LOG_TRIVIAL(debug) << "(F) " << SS(dnf);
		return dnf;
	}

	tau_<BAs...> simplify(const tau_<BAs...>& form) const {
		std::set<tau_<BAs...>> clauses;
...
template<typename...BAs>
tau_<BAs...> operator|(const tau_<BAs...>& n, const reduce_bf_t<BAs...>& r) {
	return r(n);
}
...
template<typename...BAs>
tau_<BAs...> operator|(const tau_<BAs...>& n, const reduce_wff_t<BAs...>& r) {
	return r(n);
}

// return the inner quantifier or the top wff if the formula is not quantified
template<typename...BAs>
std::pair<std::optional<tau_<BAs...>>, tau_<BAs...>> get_inner_quantified_wff(const tau_<BAs...>& n) {
	...
	if (auto quantifier = find_bottom(n, quantified); quantifier) {
		return { quantifier | tau_parser::variable,
			quantifier | tau_parser::wff | optional_value_extractor<tau_<BAs...>> };
	}
	return { {}, n };
}
...
private:
	tau_<BAs...> onf_subformula(const tau_<BAs...>& n) const {
...
template<typename...BAs>
tau_<BAs...> operator|(const tau_<BAs...>& n, const onf_wff_t<BAs...>& r) {
	return r(n);
}

template<typename... BAs>
static const auto is_not_eq_or_neq_to_zero_predicate = [](const tau_<BAs...>& n) {
...
template<typename...BAs>
std::optional<tau_<BAs...>> onf(const tau_<BAs...>& n, const tau_<BAs...>& var) {
	...
	const int_t p, const auto& is_var)
{
	...
	std::set<tau_<BAs...>> cur_vars(std::make_move_iterator(cvars.begin()),
...
template <typename... BAs>
tau_<BAs...> reduce(const tau_<BAs...>& fm, size_t type, bool is_cnf = false,
	bool all_reductions = true, bool enable_sort = true);

// Create assignment in formula and reduce resulting clause
template<typename... BAs>
bool assign_and_reduce(const tau_<BAs...>& fm,
	const std::vector<tau_<BAs...>>& vars,
	...
	bool is_wff = false)
{
	...
	if((int_t)vars.size() == p) {
		...
		if (!is_wff) {
			...
			fm_simp = to_dnf<false>(fm_simp);
			if (fm_simp == _0<BAs...>) return false;
			fm_simp = reduce(fm_simp, tau_parser::bf);
			if (fm_simp == _0<BAs...>) return false;
		} else {
			...
			fm_simp = reduce(fm_simp, tau_parser::wff);
			if (fm_simp == _F<BAs...>) return false;
		}
		if(std::ranges::all_of(i, [](const auto el) {return el == 2;})) {
...
template<typename... BAs>
tau_<BAs...> bf_boole_normal_form (const tau_<BAs...>& fm,
...
template<typename... BAs>
struct bf_reduce_canonical {
	tau_<BAs...> operator() (const tau_<BAs...>& fm) const {
		std::map<tau_<BAs...>, tau_<BAs...>> changes = {};
...
template<typename... BAs>
tau_<BAs...> operator|(const tau_<BAs...>& fm, const bf_reduce_canonical<BAs...>& r) {
	return r(fm);
}

template<typename... BAs>
std::optional<tau_<BAs...>> operator|(const std::optional<tau_<BAs...>>& fm, const bf_reduce_canonical<BAs...>& r) {
	return fm.has_value() ? r(fm.value()) : std::optional<tau_<BAs...>>{};
}
...
template<typename... BAs>
std::pair<std::vector<int_t>, bool> clause_to_vector(const tau_<BAs...>& clause,
	const auto& var_pos, const bool wff, const bool is_cnf)
{
	...
	auto var_assigner = [&](const tau_<BAs...>& n) {
	...
	rewriter::depreciating::pre_order(clause).visit_unique(var_assigner);
	return std::make_pair(move(i), clause_is_decided);
}

template<typename... BAs>
std::vector<std::vector<int_t>> collect_paths(const tau_<BAs...>& new_fm, bool wff,
...
template<typename... BAs>
tau_<BAs...> build_reduced_formula (const auto& paths, const auto& vars, bool is_cnf, bool wff) {
	if (paths.empty()) return is_cnf
					  ? (wff ? _T<BAs...> : _1<BAs...>)
					  : (wff ? _F<BAs...> : _0<BAs...>);
	tau_<BAs...> reduced_fm = is_cnf
					 ? (wff ? _F<BAs...> : _0<BAs...>)
					 : (wff ? _T<BAs...> : _1<BAs...>);
	bool first = true;
	for (const auto& path : paths) {
	...
	return from_mnf_to_nnf(reduced_fm);
}
...
tau_<BAs...> sort_var (const tau_<BAs...>& var) {
...
#endif // TAU_CACHE_DEPRECIATING
	std::stringstream ss;
	BOOST_LOG_TRIVIAL(trace) << "(F) sort var: " << SS(var);
	if (is_child_non_terminal(tau_parser::bf_eq, var)) {
		...
		for (const auto& c : clauses) {
			BOOST_LOG_TRIVIAL(trace) << "(F) c: " << SS(c) << "\n";
			...
			BOOST_LOG_TRIVIAL(trace) << "(F) res: " << SS(res) << "\n";
		}
...
template<typename... BAs>
std::pair<std::vector<std::vector<int_t>>, std::vector<tau_<BAs...>>> dnf_cnf_to_bdd(
	const tau_<BAs...>& fm, size_t type, bool is_cnf = false,
	bool all_reductions = true, bool enable_sort = true)
{
	...
	std::stringstream ss;
	BOOST_LOG_TRIVIAL(trace) << "(F) new_fm: " << SS(new_fm);
	...
	if (wff) {
		...
		for (const auto& eq : eqs) {
			...
			BOOST_LOG_TRIVIAL(trace) << "(F) sorted_eq: " << SS(sorted_eq);
			if (sorted_eq != eq) {
...
tau_<BAs...> group_dnf_expression (const tau_<BAs...>& fm) {
	...
	BOOST_LOG_TRIVIAL(debug) << "(F) Expression to factor: " << SS(fm);
	...
	std::vector<std::vector<tau_<BAs...>>> atoms_of_clauses;
	...
	tau_<BAs...> grouped_fm = wff ? _F<BAs...> : _0<BAs...>;
	for (int_t i = 0; i < (int_t)atoms_of_clauses.size(); ++i) {
		...
		std::vector<tau_<BAs...>> common;
		std::ranges::set_intersection(atoms_of_clauses[i],
					 atoms_of_clauses[max_common.first],
					 back_inserter(common));

		tau_<BAs...> cl1 = wff ? _T<BAs...> : _1<BAs...>;
		tau_<BAs...> cl2 = wff ? _T<BAs...> : _1<BAs...>;
		size_t p = 0;
		...
		tau_<BAs...> grouped = wff ? build_wff_or(cl1, cl2) :
	...
	BOOST_LOG_TRIVIAL(debug) << "(F) Factored expression: " << SS(grouped_fm);
	return grouped_fm;
}
...
template<typename... BAs>
tau_<BAs...> simp_general_excluded_middle (const tau_<BAs...>& fm) {
	assert(is_non_terminal(tau_parser::bf, fm));
	...
	for (int_t i = 0; i < (int_t)clauses.size(); ++i) {
		...
		auto cn = push_negation_in<false>(build_bf_neg(clauses[i]));
	...
	return was_simplified ? to_dnf<false>(build_bf_or<BAs...>(clauses)) : fm;
}
...
tau_<BAs...> reduce(const tau_<BAs...>& fm, size_t type, bool is_cnf,
	...
	BOOST_LOG_TRIVIAL(trace) << "(F) Formula to reduce: " << SS(fm);
	...
	BOOST_LOG_TRIVIAL(trace) << "(F) Reduced formula: " << SS(reduced_fm);
	return reduced_fm;
}

template<typename... BAs>
tau_<BAs...> reduce_terms (const tau_<BAs...>& fm, bool with_sorting = false) {
    std::map<tau_<BAs...>, tau_<BAs...>> changes = {};
    for (const auto& bf: select_top(fm, is_non_terminal<tau_parser::bf, BAs...>)) {
...
template<typename... BAs>
tau_<BAs...> apply_eqs_across_clauses (const tau_<BAs...>& fm) {
	std::vector<tau_<BAs...>> eq_reductions, neq_reductions;
...
template<typename... BAs>
std::vector<tau_<BAs...>> push_eq_and_get_vars (tau_<BAs...>& fm) {
...
template<typename... BAs>
std::vector<std::vector<int_t>> wff_to_bdd (const tau_<BAs...>& fm, auto& vars = {}) {
	// Find atomic formulas
	...
	std::map<tau_<BAs...>, std::vector<std::vector<int_t>>> dnf;
...
template<typename... BAs>
std::vector<std::vector<std::vector<tau_<BAs...>>>> get_cnf_inequality_lits(
	const tau_<BAs...>& fm)
{
	auto neq_pushed_in = unsqueeze_wff_neg(fm);
	if (neq_pushed_in == _T<BAs...>) return {};
	// cout << "neq_pushed_in: " << neq_pushed_in << "\n";
	std::vector<std::vector<std::vector<tau_<BAs...>>>> cnf_lits;
	for (const tau_<BAs...>& clause : get_cnf_wff_clauses(neq_pushed_in)) {
		std::vector<std::vector<tau_<BAs...>>> c;
		for (const tau_<BAs...>& neqs : get_dnf_wff_clauses(clause)) {
...
std::pair<std::vector<int_t>, bool> simplify_path(
	const std::vector<int_t>& path, std::vector<tau_<BAs...> >& vars)
{
	// std::cout << vars << "\n";
	using tp = tau_parser;
	std::vector<std::vector<tau_<BAs...>>> pos;
	// Build clause for non-equality terms
	// and new representation for equality terms
	tau_<BAs...> clause = _T<BAs...>;
	tau_<BAs...> pos_bf = _0<BAs...>;
	tau_<BAs...> negs_wff = _T<BAs...>;
	...
	if (neq_cnf) {
		...
		neq_cnf = reduce(neq_cnf, tau_parser::wff, true, true, false);
		// std::cout << "neq_cnf after reduce: " << neq_cnf << "\n";
		neq_cnf = neq_cnf |(tau_f<BAs...>) squeeze_wff_neg<BAs...>
				|(tau_f<BAs...>) to_mnf<BAs...>;
...
template<typename...BAs>
std::pair<tau_<BAs...>, bool> group_paths_and_simplify(
	std::vector<std::vector<int_t> >& paths, const auto& vars)
{
	...
	std::stringstream ss;
	tau_<BAs...> result = _F<BAs...>;
	for (size_t i = 0; i < paths.size(); ++i) {
		// Now add from paths[i] and groups[i]
		tau_<BAs...> neqs = neq_from_path(paths[i]);
		BOOST_LOG_TRIVIAL(debug) << "(F) neqs: " << SS(neqs);
		...
		for (auto& neq : neq_clauses) {
			BOOST_LOG_TRIVIAL(debug) << "(F) neq: " << SS(neq);
	...
	BOOST_LOG_TRIVIAL(debug) << "(F) result: " << SS(result);
	BOOST_LOG_TRIVIAL(debug) << "(F) is_simp: " << is_simp;
	return make_pair(result, is_simp);
}

template<typename... BAs>
tau_<BAs...> reduce_across_bfs (const tau_<BAs...>& fm, bool to_cnf) {
	...
	BOOST_LOG_TRIVIAL(debug) << "(I) Formula in DNF: " << SS(squeezed_fm);
#ifdef TAU_CACHE_DEPRECIATING
		static std::map<std::pair<tau_<BAs...>, bool>, tau_<BAs...>> cache;
	...
	for (const auto& v : vars)
		BOOST_LOG_TRIVIAL(trace) << "(F) v: " << SS(v);
	ss.str({});
	for (const auto& p : paths) {
		ss << "\tpath:\t";
		for (const auto& i : p) ss << i << " ";
	...
	if (paths.empty()) {
		...
		BOOST_LOG_TRIVIAL(debug) << "(F) " << SS(res);
		return res;
	...
	while (true) {
		...
		if (has_further_simp) {
			BOOST_LOG_TRIVIAL(debug) << "(F) Before factoring simplification: "
				<< SS(build_reduced_formula<BAs...>(paths, vars, false, true));
			BOOST_LOG_TRIVIAL(debug) << "(F) After factoring simplification: " << SS(simp_fm);
		}
#endif // DEBUG
...
template<typename... BAs>
tau_<BAs...> operator|(const tau_<BAs...>& fm, const wff_reduce_dnf<BAs...>& r) {
	return r(fm);
}

template<typename... BAs>
tau_<BAs...> operator|(const tau_<BAs...>& fm, const wff_reduce_cnf<BAs...>& r) {
	return r(fm);
}

template<typename... BAs>
tau_<BAs...> conjunct_dnfs_to_dnf (const tau_<BAs...>& d1, const tau_<BAs...>& d2) {
...
template<typename... BAs>
tau_<BAs...> disjunct_cnfs_to_cnf (const tau_<BAs...>& c1, const tau_<BAs...>& c2) {
...
template<bool is_wff = true, typename... BAs>
tau_<BAs...> push_negation_one_in(const tau_<BAs...>& fm) {
	using p = tau_parser;
	// Tau formula rules
	...
	if constexpr (!is_wff) if (is_child_non_terminal(p::bf_neg, fm)) {
		auto c = trim2(fm);
...
tau_<BAs...> push_negation_in(const tau_<BAs...>& fm) {
	auto pn = [](const auto& n) {
		return push_negation_one_in<is_wff>(n);
	};
	if constexpr (is_wff) return rewriter::depreciating::pre_order(fm).template
	apply_unique<MemorySlotPre::push_negation_in_m>(
		pn, visit_wff<BAs...>, rewriter::depreciating::identity);
	else return rewriter::depreciating::pre_order(fm).template
	apply_unique<MemorySlotPre::push_negation_in_m>(
		pn, rewriter::depreciating::all, rewriter::depreciating::identity);
}

// Conversion to dnf while applying reductions during the process
template<bool is_wff, typename... BAs>
tau_<BAs...> to_dnf(const tau_<BAs...>& fm) {
	using p = tau_parser;
	auto layer_to_dnf = [](const auto& n) {
		...
		if constexpr (!is_wff) if (is_non_terminal(p::bf, n)) {
	...
	apply_unique<MemorySlotPre::to_dnf2_m>(
		pn, rewriter::depreciating::all, layer_to_dnf);
}

template<typename... BAs>
tau_<BAs...> single_dnf_lift(const tau_<BAs...>& fm) {
	using p = tau_parser;
	auto layer_to_dnf = [](const auto& n) {
		if (is_child_non_terminal(p::wff_and, n)) {
	...
	return rewriter::depreciating::post_order(fm).apply_unique(layer_to_dnf, decend);
}

// Conversion to cnf while applying reductions during the process
template<bool is_wff, typename... BAs>
tau_<BAs...> to_cnf(const tau_<BAs...>& fm) {
	using p = tau_parser;
	auto layer_to_cnf = [](const auto& n) {
		...
		if constexpr (!is_wff) if (is_child_non_terminal(p::bf_or, n)) {
				auto dis = disjunct_cnfs_to_cnf(
					trim(n)->child[0],
					trim(n)->child[1]);
				// Perform simplification
				if (dis != n)
					return reduce(dis, p::bf, true);
				else return n;
			}
		return n;
	};
	...
	apply_unique<MemorySlotPre::to_cnf2_m>(
		pn, rewriter::depreciating::all, layer_to_cnf);
}

// Assumes that fm is a single DNF always clause
template<typename... BAs>
tau_<BAs...> rm_temporary_lookback (const tau_<BAs...>& fm) {
	auto io_vars = rewriter::depreciating::select_top(fm,
...
template<typename... BAs>
tau_<BAs...> extract_sometimes (tau_<BAs...> fm) {
	std::map<tau_<BAs...>, tau_<BAs...>> l_changes = {};
	// Collect remaining nested "sometimes" formulas
	std::vector<tau_<BAs...>> sometimes_extractions = {};
	...
	std::vector<tau_<BAs...>> always_extractions = {};
	...
	std::vector<tau_<BAs...>> extracted = {}, staying = {};
...
template<typename... BAs>
tau_<BAs...> extract_always (tau_<BAs...> fm) {
	std::map<tau_<BAs...>, tau_<BAs...>> l_changes = {};
	// Collect remaining nested "sometimes" formulas
	std::vector<tau_<BAs...>> sometimes_extractions = {};
	for (const auto &inner_st: select_top(trim2(fm), is_child_non_terminal<tau_parser::wff_sometimes, BAs...>)) {
...
template<typename... BAs>
tau_<BAs...> push_sometimes_always_in (tau_<BAs...> fm, auto& visited) {
	std::map<tau_<BAs...>, tau_<BAs...>> g_changes = {};
	for (const auto &st : select_top_until(fm, is_child_non_terminal<tau_parser::wff_sometimes, BAs...>,
		...
		flat_st = build_wff_sometimes(to_dnf(flat_st));
		flat_st = push_temp_in(flat_st, tau_parser::wff_sometimes, visited);
		flat_st = reduce(flat_st, tau_parser::wff);
	...
	for (const auto& aw : select_top_until(fm, is_child_non_terminal<tau_parser::wff_always, BAs...>,
		...
		// Reductions done in order to prevent blow up
		flat_aw = build_wff_always(to_cnf(flat_aw));
		...
		flat_aw = reduce(flat_aw, tau_parser::wff, true);
...
template<typename... BAs>
tau_<BAs...> shift_io_vars_in_fm (const tau_<BAs...>& fm, const auto& io_vars, const int_t shift) {
...
template<typename... BAs>
tau_<BAs...> shift_const_io_vars_in_fm(const tau_<BAs...>& fm,
...
template<typename... BAs>
tau_<BAs...> pull_always_out(const tau_<BAs...>& fm) {
	std::map<tau_<BAs...>, tau_<BAs...>> l_changes = {};
	std::vector<tau_<BAs...>> collected_always_fms;
...
template<typename... BAs>
tau_<BAs...> pull_sometimes_always_out(tau_<BAs...> fm) {
	std::map<tau_<BAs...>, tau_<BAs...>> changes = {};
	std::vector<tau_<BAs...>> collected_no_temp_fms;
	tau_<BAs...> pure_always_clause;
...
struct sometimes_always_normalization_depreciated {
	tau_<BAs...> operator() (const tau_<BAs...>& fm) const {
		...
		tau_<BAs...> res = !st_aw(fm) ? build_wff_always(fm) : fm;
...
template<typename...BAs>
tau_<BAs...> always_conjunction (const tau_<BAs...>& fm1_aw, const tau_<BAs...>& fm2_aw) {
	using p = tau_parser;
...
struct sometimes_always_normalization {
	tau_<BAs...> operator() (const tau_<BAs...>& fm) const {
		...
		tau_<BAs...> res = _F<BAs...>;
		tau_<BAs...> always_disjuncts = _F<BAs...>;
		for (const tau_<BAs...>& clause : clauses) {
			...
			tau_<BAs...> always_part = _T<BAs...>;
			tau_<BAs...> staying = _T<BAs...>;
			for (const tau_<BAs...>& conj : conjuncts) {
				if (!st_aw(conj))
...
template<typename... BAs>
tau_<BAs...> pull_always_out_for_inf (const tau_<BAs...>& fm) {
	using p = tau_parser;
	// Get all always statments on each clause and merge
	auto clauses = get_dnf_wff_clauses(fm);
	tau_<BAs...> res = _F<BAs...>;
	tau_<BAs...> non_temps = _F<BAs...>;
	tau_<BAs...> last_always;
	for (const tau_<BAs...>& clause : clauses) {
		...
		auto conjuncts = get_cnf_wff_clauses(clause);
		tau_<BAs...> always_statements = _T<BAs...>;
		tau_<BAs...> staying = _T<BAs...>;
		for (const tau_<BAs...>& conj : conjuncts) {
...
template<typename... BAs>
tau_<BAs...> operator|(const tau_<BAs...>& fm, const sometimes_always_normalization<BAs...>& r) {
	return r(fm);
}

template <typename... BAs>
tau_<BAs...> push_existential_quantifier_one (const tau_<BAs...>& fm) {
	using p = tau_parser;
	assert(is_child_non_terminal(p::wff_ex, fm));
	const tau_<BAs...> scoped_fm = trim(fm)->child[1];
	const tau_<BAs...> quant_var = trim2(fm);
	...
	if (is_child_non_terminal(p::wff_or, scoped_fm)) {
		...
		tau_<BAs...> no_q_fm = _T<BAs...>;
		for (tau_<BAs...>& clause : clauses) {
...
template <typename... BAs>
tau_<BAs...> push_universal_quantifier_one (const tau_<BAs...>& fm) {
	using p = tau_parser;
...
template <typename... BAs>
std::optional<tau_<BAs...>> squeeze_positives(const tau_<BAs...>& n) {
...
template<typename... BAs>
tau_<BAs...> wff_remove_existential(const tau_<BAs...>& var, const tau_<BAs...>& wff) {
	...
	for (const auto& l: get_leaves(wff, tau_parser::wff_or)) {
		...
		if (auto neqs = select_all(new_l, is_non_terminal<tau_parser::bf_neq, BAs...>); neqs.size() > 0) {
			auto nneqs = _T<BAs...>;
			for (auto& neq: neqs) {
				auto g = neq | tau_parser::bf | optional_value_extractor<tau_<BAs...>>;
	...
	return replace<tau_<BAs...>>(wff, changes);
}

template<typename... BAs>
tau_<BAs...> eliminate_existential_quantifier(const auto& inner_fm, auto& scoped_fm) {
	...
	BOOST_LOG_TRIVIAL(debug) << "(F) Quantified formula: " << SS(scoped_fm);
...
template<typename... BAs>
tau_<BAs...> eliminate_universal_quantifier(const auto& inner_fm, auto& scoped_fm) {
	std::stringstream ss;
...
template<typename... BAs>
tau_<BAs...> eliminate_quantifiers(const tau_<BAs...>& fm) {
	// Lambda is applied to nodes of fm in post order after quantifiers have
	// been pushed in
	auto elim_quant = [](const tau_<BAs...>& inner_fm) -> tau_<BAs...> {
	...
	auto push_quantifiers = [&excluded_nodes](const tau_<BAs...>& n) {
		using p = tau_parser;
	...
	auto visit = [&excluded_nodes](const tau_<BAs...>& n) {
		using p = tau_parser;
	...
	auto push_and_elim = [&elim_quant, &push_quantifiers, visit](const tau_<BAs...>& n) {
	...
	return rewriter::depreciating::post_order(fm).apply_unique(push_and_elim, visit_wff<BAs...>);
}

// fm is assumed to be quantifier free
template <typename... BAs>
tau_<BAs...> get_eq_with_most_quant_vars (const tau_<BAs...>& fm, const auto& quant_vars) {
	using p = tau_parser;
	// std::cout << "Begin get_eq_with_most_quant_vars with\n";
	// std::cout << "fm: " << fm << "\n";
	// std::cout << "quantified vars: " << quant_vars << "\n";
	tau_<BAs...> eq_max_quants;
	int_t max_quants = 0;
	auto get_eq = [&](const tau_<BAs...>& n) {
		if (is_non_terminal(p::bf_eq, n)) {
			// Found term
			// Get vars
...
rewriter::depreciating::pre_order(fm).visit(get_eq, visit_wff<BAs...>, rewriter::depreciating::identity);
	return wrap(p::wff, eq_max_quants);
}

template <typename... BAs>
std::pair<tau_<BAs...>, bool> anti_prenex_finalize_ex (const tau_<BAs...>& q, const tau_<BAs...>& scoped_fm) {
	using p = tau_parser;
	...
	auto check_atm_fms = [&all_atm_fm_neq, &all_atm_fm_eq, &q](const tau_<BAs...>& n) {
...
rewriter::depreciating::pre_order(scoped_fm).search_unique(check_atm_fms, visit_wff<BAs...>, rewriter::depreciating::identity);
	if (all_atm_fm_neq) {
		// std::cout << "all atomic fms are negative\n";
		// All atomic formulas are of the form !=
		auto elim_quants = [&q](const tau_<BAs...>& n) {
			if (is_child_non_terminal(p::bf_neq, n) && contains(n, q)) {
				return wff_remove_existential(q, n);
			} else return n;
		};
		return {rewriter::depreciating::pre_order(scoped_fm).apply_unique_until_change(
			elim_quants, visit_wff<BAs...>, rewriter::depreciating::identity), true};
	} else if (all_atm_fm_eq) {
		// std::cout << "all atomic fms are positive\n";
		tau_<BAs...> red = reduce_across_bfs(scoped_fm, false);
...
template <typename... BAs>
tau_<BAs...> anti_prenex (const tau_<BAs...>& fm) {
	unordered_tau_set<BAs...> excluded_nodes, quant_vars;
	auto anti_prenex_step = [&excluded_nodes](const tau_<BAs...>& n) {
		if (is_child_quantifier<BAs...>(n)) {
			...
			tau_<BAs...> scoped_fm = trim(n)->child[1];
			...
			tau_<BAs...> res = eliminate_existential_quantifier<BAs...>(n, scoped_fm);
	...
	auto inner_quant = [&anti_prenex_step, &visit, &quant_vars](const tau_<BAs...>& n) {
		if (is_child_quantifier<BAs...>(n)) {
			...
			if (is_child_non_terminal(p::wff_all, n)) {
				...
				auto res = rewriter::depreciating::pre_order(n_neg).template
				apply_unique<MemorySlotPre::anti_prenex_step_m>(
					anti_prenex_step, visit, rewriter::depreciating::identity);
				...
				// std::cout << "Before elimination:\n";
				// std::cout << n << "\n\n";
				auto res = rewriter::depreciating::pre_order(n).template
				apply_unique<MemorySlotPre::anti_prenex_step_m>(
					anti_prenex_step, visit, rewriter::depreciating::identity);
				quant_vars.erase(trim2(n));
				// std::cout << "After elimination:\n";
	...
	auto visit_inner_quant = [&quant_vars](const tau_<BAs...>& n) {
		using p = tau_parser;
	...
	return rewriter::depreciating::post_order(nnf).template
	apply_unique<MemorySlotPost::anti_prenex_m>(inner_quant, visit_inner_quant);
}

template <typename... BAs>
tau_<BAs...> replace_free_vars_by (const tau_<BAs...>& fm, const tau_<BAs...>& val) {
	assert(!is_non_terminal(tau_parser::bf, val));
	auto free_vars = get_free_vars_from_nso(fm);
	if (!free_vars.empty()) {
		std::map<tau_<BAs...>, tau_<BAs...>> free_var_assgm;
...
struct to_snf_step {
	...
	tau_<BAs...> operator()(const tau_<BAs...>& form) const {
		...
		if (auto literals = select_all(form, is_literal); !literals.empty()) {
			...
			// of the BDD.
			std::set<tau_<BAs...>> remaining(literals.begin(), literals.end());
			bdd_path path;
...
private:
	...
	tau_<BAs...> bdd_path_to_snf(const bdd_path& path, const tau_<BAs...>& form) const {
	...
	exponent get_exponent(const tau_<BAs...>& n) const {
	...
	tau_<BAs...> squeeze_positives(const std::set<literal>& positives, const exponent& exp) const {
		...
		if (first == positives.end()) return tau_<BAs...>(*positives.begin());
		// otherwise...
	...
	tau_<BAs...> normalize(const bdd_path& path) const {
	...
	tau_<BAs...> normalize_positive(const bdd_path& path, const literal& positive) const {
		auto relative_path = get_relative_path(path, positive);
		return normalize(add_to_positive_path(relative_path, positive));
	}

	tau_<BAs...> normalize_negative(const bdd_path& path, const literal& negative) const {
...
template<typename...BAs>
tau_<BAs...> operator|(const tau_<BAs...>& n, const to_snf_step<BAs...>& r) {
	return r(n);
}

template<typename...BAs>
tau_<BAs...> snf_bf(const tau_<BAs...>& n) {
...
template<typename...BAs>
tau_<BAs...> snf_wff(const tau_<BAs...>& n) {
...
template<typename...BAs>
tau_<BAs...> build_split_wff_using(tau_parser::nonterminal type, const tau_<BAs...>& a, const tau_<BAs...>& b) {
...
#endif // __NORMAL_FORMS_DEPRECIATING_H__