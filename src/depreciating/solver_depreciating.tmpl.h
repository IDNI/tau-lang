...
std::optional<solution<BAs...>> find_solution(const equality<BAs...>& eq, solution<BAs...>& substitution, solver_mode mode) {
	...
	auto has_no_var = [](const tau_<BAs...>& f) {
	...
	auto f = eq | tau_parser::bf_eq | tau_parser::bf | optional_value_extractor<tau_<BAs...>>;
...
template<typename...BAs>
std::vector<tau_<BAs...>> get_variables(const equality<BAs...>& eq) {
	return select_top(eq, is_child_non_terminal<tau_parser::variable, BAs...>);
}

template<typename...BAs>
std::vector<tau_<BAs...>> get_variables(const equation_system<BAs...>& system) {
	std::vector<tau_<BAs...>> vars;
...
public:
	...
	minterm_iterator(const tau_<BAs...>& f) {
		// FIXME convert vars to a set
		if (auto vars = select_top(f, is_child_non_terminal<tau_parser::variable, BAs...>); !vars.empty()) {
...
public:
	...
	explicit minterm_range(const tau_<BAs...>& f): f (f) {}
...
public:
	...
	minterm_inequality_system_iterator(const inequality_system<BAs...>& sys) {
		...
		for (auto& neq: sys) {
			auto f = neq | tau_parser::bf_neq | tau_parser::bf | optional_value_extractor<tau_<BAs...>>;
...
template<typename...BAs>
tau_<BAs...> get_constant(const minterm<BAs...>& m) {
...
template<typename...BAs>
std::set<tau_<BAs...>> get_exponent(const tau_<BAs...>& n) {
	...
	return std::set<tau_<BAs...>>(all_vs.begin(), all_vs.end());
}

template<typename...BAs>
tau_<BAs...> get_minterm(const minterm<BAs...>& m) {
...
std::optional<solution<BAs...>> solve(const tau_<BAs...>& form,
	...
	for (auto& clause: get_leaves(dnf, tau_parser::wff_or)) {
		...
		auto is_equation = [](const tau_<BAs...>& n) {
		...
		auto solution = solve<BAs...>(
			std::set<tau_<BAs...>>(eqs.begin(), eqs.end()), options);
		if (solution.has_value()) return solution;
	}
	return {};
}
