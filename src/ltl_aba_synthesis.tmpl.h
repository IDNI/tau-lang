...
// parser/hoa_parser.generated.{h,cpp}); this walker holds only what a
// grammar cannot express -- numeric range checks and the LT-10 drops.
// The grammar mirrors the tolerance of the previous hand-written line
// parser: unknown lines match a catch-all and are ignored here, a header
// keyword whose value is not a plain number falls into the catch-all too
// (so `Start: garbage` leaves the default and a garbled `States:` count is
...
inline HoaAutomaton parse_hoa(const std::string& hoa_text) {
	...
	// program.
	auto result = hoa_parser::instance().parse(
					hoa_text.c_str(), hoa_text.size());
	if (!result.found)
		throw ltl_synthesis_error("malformed HOA strategy: not a HOA "
			"automaton (truncated, or no `--BODY--`)");
	auto root = tt(result.get_shaped_tree2());
	...
	auto num_of = [](const tt& n) -> long {
		try {
			return std::stol(n | hoa::num | tt::terminals);
	...
	if (!seen_states)
		throw ltl_synthesis_error("malformed HOA strategy: "
			"no `States:` header");
