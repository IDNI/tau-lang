...
template<typename B, bool inv_in = true, bool inv_out = true, bool varshift = false>
hbdd<B, inv_in, inv_out, varshift>
operator&(const hbdd<B, inv_in, inv_out, varshift>& x,
...
template<typename B, bool inv_in = true, bool inv_out = true, bool varshift = false>
hbdd<B, inv_in, inv_out, varshift>
        operator~(const hbdd<B, inv_in, inv_out, varshift>& x) { return ~(*x); }
...
struct bdd_handle {
	...
	inline static hbdd<B, inv_in, inv_out, varshift> htrue, hfalse;
...
//	bdd_handle();
	...
	static hbdd<B, inv_in, inv_out, varshift> get(const bdd_node& x) {
		if (auto it = Mn.find(x); it != Mn.end())
			return it->second;//.lock();
		hbdd<B, inv_in, inv_out, varshift> h =
			make_shared<bdd_handle<B, inv_in, inv_out, varshift>>(); //(new bdd_handle);
		return h->b = bdd<B, inv_in, inv_out, varshift>::add(x), Mn.emplace(x, h), h;
	}

	static hbdd<B, inv_in, inv_out, varshift> get(const B& x) {
	...
	static hbdd<B, inv_in, inv_out, varshift>
	        get(const bdd<B, inv_in, inv_out, varshift>& x) {
		return	x.leaf() ? get(std::get<B>(x))
			: get(std::get<bdd_node>(x));
	}

	static hbdd<B, inv_in, inv_out, varshift> get(int_t t) {
	...
	static hbdd<B, inv_in, inv_out, varshift> one() {
		return get(bdd<B, inv_in, inv_out, varshift>::T);
	}

	static hbdd<B, inv_in, inv_out, varshift> zero() {
		return get(bdd<B, inv_in, inv_out, varshift>::F);
	}

	static hbdd<B, inv_in, inv_out, varshift> bit(bool b, int_t v) {
		DBG(assert(v > 0);)
		hbdd<B, inv_in, inv_out, varshift> r =
			get(bdd<B, inv_in, inv_out, varshift>::bit(b ? v : -v));
		//hbdd<B, inv_in, inv_out, varshift> r = get(bdd_node(v, bdd<B, inv_in, inv_out, varshift>::T, bdd<B, inv_in, inv_out, varshift>::F));
		DBG(assert(r);)
		return r;
	}
	...
	hbdd<B, inv_in, inv_out, varshift> operator&(
		const hbdd<B, inv_in, inv_out, varshift>& x) const {
	...
	hbdd<B, inv_in, inv_out, varshift> operator~() const {
		return get( bdd<B, inv_in, inv_out, varshift>::bdd_and(
			bdd<B, inv_in, inv_out, varshift>::T,
			bdd<B, inv_in, inv_out, varshift>::bdd_not(b)));
	}

	hbdd<B, inv_in, inv_out, varshift>
	operator|(const hbdd<B, inv_in, inv_out, varshift>& x) const {
		return ~((~x) & (~*this));
	}

	hbdd<B, inv_in, inv_out, varshift> ex(int_t v) const {
		return get(bdd<B, inv_in, inv_out, varshift>::ex(b, v));
	}

	hbdd<B, inv_in, inv_out, varshift> all(int_t v) const {
		return get(bdd<B, inv_in, inv_out, varshift>::all(b, v));
	}

	hbdd<B, inv_in, inv_out, varshift>
	subst(size_t v, const hbdd<B, inv_in, inv_out, varshift>& x) const {
#ifdef DEBUG
...
//		return get(bdd<B, inv_in, inv_out, varshift>::subst(b, v, x->b));
	...
	hbdd<B, inv_in, inv_out, varshift> sub0(size_t v) const {
		return get(bdd<B, inv_in, inv_out, varshift>::sub0(b, v));
	}

	hbdd<B, inv_in, inv_out, varshift> sub1(size_t v) const {
		return get(bdd<B, inv_in, inv_out, varshift>::sub1(b, v));
	}

	hbdd<B, inv_in, inv_out, varshift>
	condition(size_t v, const hbdd<B, inv_in, inv_out, varshift>& f) const {
...
//#endif
	...
	hbdd<B, inv_in, inv_out, varshift>
	compose(const map<int_t, hbdd<B, inv_in, inv_out, varshift>>& m) const {
		map<int_t, int_t> p;
	...
	map<int_t, hbdd<B, inv_in, inv_out, varshift>> lgrs() const {
		map<int_t, hbdd<B, inv_in, inv_out, varshift>> r;
...
template<typename B, bool inv_in = true, bool inv_out = true, bool varshift = false> void bdd_init() {
	if (!bdd<B, inv_in, inv_out, varshift>::V.empty()) return;
#ifdef DEBUG
	int s;
	cout << "bdd_init" << '<' <<
	...
	} else bdd<B, inv_in, inv_out, varshift>::V.emplace_back(one);
	bdd<B, inv_in, inv_out, varshift>::V.emplace_back(one);
	bdd<B, inv_in, inv_out, varshift>::Mb.emplace(one, 1);
	//bdd<B, inv_in, inv_out, varshift>::Mb.emplace(B::one(), 1);
	if(inv_out) bdd<B, inv_in, inv_out, varshift>::F =
